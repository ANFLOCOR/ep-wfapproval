...
Public Class WFinRep_HeadTableControlRow
        ...
        Public Overrides Sub DataBind()
            ...
            For Each r As String In roles
                If r = "151" Then '157 - old FS Viewer
                    rolecorrect = True
                End If
                If r = "147" Then '160 - old FS Admin
                    isAdmin = True
                End If
            Next r

            If Me.HFIN_Status.Text = "Completed" And rolecorrect = True Then
            ...
            Next r
            ''101 OVG
            ''158 Approver2 as Final Approver
            ...
            End If
            If isAdmin Then
                ' ''Me.btnGenPDF.Visible = True
            Else
                ' ''Me.btnGenPDF.Visible = False
            End If
        ...
        Public Overrides Sub btnGenPDF_Click(ByVal sender As Object, ByVal args As EventArgs)

            Try
                Dim fileName As String = ""
                Dim databaseName As String = ""
                Select Case Me.HFIN_C_ID1.Text
                    Case "ANFLOCOR"
                        databaseName = "AMIC_DW_WF"
                    Case "UNIFINANCE"
                        databaseName = "UNIFC_DW_WF"
                    Case "DAVCO"
                        databaseName = "DAVC_DW_WF"
                    Case "PITRADE"
                        databaseName = "PITRD_DW_WF"
                    Case Else
                        databaseName = Me.HFIN_C_ID1.Text & "_DW_WF"
                End Select
                If Me.HFIN_C_ID1.Text = "ANFLOCOR" Then
                    fileName = "/ANFLOCOR/Reports/Finance/FS Package/" & Me.HFIN_C_ID.Text & " FS Package"
                Else
                    fileName = "/ANFLOCOR/Reports/Finance/FS Package II/" & Me.HFIN_C_ID.Text & "/" & Me.HFIN_C_ID.Text & " FS Package"
                End If


                ' ''CreateFile(fileName, Me.HFIN_Month1.Text, Me.HFIN_Year1.Text, Me.HFIN_Status1.Text, databaseName, Me.HFIN_ID.Text)
                Me.DataChanged = True
        ...
        ' ''Public Sub CreateFile(ByVal pFileName As String, ByVal pPeriod As String, ByVal pYear As String, ByVal pStatus As String, ByVal pDatabaseName As String, ByVal pHeaderDocID As String)
        ' ''    'Dim rs As anflosouth_db.ReportingService2005 = New anflosouth_db.ReportingService2005

        ' ''    ''First: I-Add ang mga reportingservice and reportexecution from the reportserver
        ' ''    ''Sa VS : add web service:
        ' ''    'Change lang to your report server name
        ' ''    'http://anflosouth_db/ReportServer/ReportExecution2005.asmx?wsdl
        ' ''    'http://anflosouth_db/ReportServer/ReportService2005.asmx?wsdl
        ' ''    'After na na add ang web service, icheck kung maaccess na ang mga properties below

        ' ''    Dim rs As anflosouth_dev1_ReportService2005.ReportingService2005 = New anflosouth_dev1_ReportService2005.ReportingService2005
        ' ''    Dim rsExec As anflosouth_dev1_ReportExecution2005.ReportExecutionService = New anflosouth_dev1_ReportExecution2005.ReportExecutionService
        ...
        ' ''    Dim warnings As anflosouth_dev1_ReportExecution2005.Warning() = Nothing
        ' ''    Dim streamIDs As String() = Nothing
        ' ''    Dim filename As String = "C:\MyReport.pdf"        ' temporary lang to
        ' ''    ''Dim _reportName As String = "/ANFLOCOR/REPORTS/FINANCE/FS PACKAGE II/CONSOLIDATED/Statements of Financial Position"  ' Change to be your report
        ' ''    Dim _reportName As String = ""
        ' ''    _reportName = pFileName
        ' ''    Dim _historyID As String = Nothing
        ' ''    Dim _forRendering As Boolean = False
        ' ''    Dim _values As anflosouth_dev1_ReportService2005.ParameterValue() = Nothing
        ' ''    Dim _credentials As anflosouth_dev1_ReportService2005.DataSourceCredentials() = Nothing
        ' ''    Dim _parameters As anflosouth_dev1_ReportService2005.ReportParameter() = Nothing

        ' ''    _parameters = rs.GetReportParameters(_reportName, _historyID, _forRendering, _values, _credentials)

        ' ''    Dim ei As anflosouth_dev1_ReportExecution2005.ExecutionInfo = rsExec.LoadReport(_reportName, historyID)
        ' ''    Dim parameters(5) As anflosouth_dev1_ReportExecution2005.ParameterValue

        ' ''    If _parameters.Length > 0 Then
        ' ''        parameters(0) = New anflosouth_dev1_ReportExecution2005.ParameterValue
        ' ''        parameters(0).Label = "DatabaseName"
        ' ''        parameters(0).Name = "DatabaseName"
        ' ''        parameters(0).Value = pDatabaseName

        ' ''        parameters(1) = New anflosouth_dev1_ReportExecution2005.ParameterValue
        ' ''        parameters(1).Label = "User"
        ' ''        parameters(1).Name = "User"
        ' ''        parameters(1).Value = DirectCast(Me.Page, BaseApplicationPage).CurrentSecurity.GetUserStatus().ToString()

        ' ''        parameters(2) = New anflosouth_dev1_ReportExecution2005.ParameterValue
        ' ''        parameters(2).Label = "Month"
        ' ''        parameters(2).Name = "Month"
        ' ''        parameters(2).Value = pPeriod

        ' ''        parameters(3) = New anflosouth_dev1_ReportExecution2005.ParameterValue
        ' ''        parameters(3).Label = "Year"
        ' ''        parameters(3).Name = "Year"
        ' ''        parameters(3).Value = pYear

        ' ''        parameters(4) = New anflosouth_dev1_ReportExecution2005.ParameterValue
        ' ''        parameters(4).Label = "Status"
        ' ''        parameters(4).Name = "Status"
        ' ''        parameters(4).Value = pStatus

        ' ''        parameters(5) = New anflosouth_dev1_ReportExecution2005.ParameterValue
        ' ''        parameters(5).Label = "HFIN_ID"
        ' ''        parameters(5).Name = "HFIN_ID"
        ' ''        parameters(5).Value = pHeaderDocID
        ...
        ' ''    SaveToDatabase(results, CInt(pHeaderDocID))
        ' ''End Sub
