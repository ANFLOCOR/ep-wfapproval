...
' Sel_WAttach_Type_WCAR_Doc_Attach_QuickSelector.aspx page.  The Row or RecordControl classes are the 
...
Namespace ePortalWFApproval.UI.Controls.Sel_WAttach_Type_WCAR_Doc_Attach_QuickSelector
...
' Base class for the SelectorTableControlRow control on the Sel_WAttach_Type_WCAR_Doc_Attach_QuickSelector page.
...
' Base class for the SelectorTableControl control on the Sel_WAttach_Type_WCAR_Doc_Attach_QuickSelector page.
    ...
    ' Setup default pagination settings.
        ...
        Public Overridable Function GetColumnListFromURLParameters(ByVal includeIndexFieldParameter As Boolean, ByVal includeFormulaAndDFKAParameter As Boolean) As ColumnList
            ...
            If includeIndexFieldParameter AndAlso field <> "" Then
                Dim col As BaseColumn = Sel_WAttach_Type_WCAR_Doc_AttachView.GetColumnByName(field)
                If col IsNot Nothing  AndAlso Not columnList.Contains(col) Then columnList.Add(col)             
            End If
            If includeFormulaAndDFKAParameter Then
                If formula <> "" Then
                    For Each col As BaseColumn In MiscUtils.GetFormulaFieldList(formula, Sel_WAttach_Type_WCAR_Doc_AttachView.Instance.TableDefinition)
                        If col IsNot Nothing AndAlso Not columnList.Contains(col) Then columnList.Add(col)             
                    Next
                
                ElseIf displayFieldName <> ""
                    Dim col As BaseColumn = Sel_WAttach_Type_WCAR_Doc_AttachView.GetColumnByName(displayFieldName)
                    If col IsNot Nothing AndAlso Not columnList.Contains(col) Then columnList.Add(col)             
                End If
            End If
            Return columnList            
      ...
      Next
            ...
            End If
		        ...
		        If Not String.IsNullOrEmpty(Me.Page.Request("dependOnField")) AndAlso Me.Page.Request("dependOnVal") IsNot Nothing Then
			          Dim dependOnCol as BaseColumn = Sel_WAttach_Type_WCAR_Doc_AttachView.GetColumnByName(Me.Page.GetDecryptedURLParameter("dependOnField"))
