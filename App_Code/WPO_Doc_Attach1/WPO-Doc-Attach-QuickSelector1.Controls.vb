...
' WPO_Doc_Attach_QuickSelector1.aspx page.  The Row or RecordControl classes are the 
...
Namespace ePortalWFApproval.UI.Controls.WPO_Doc_Attach_QuickSelector1
...
' Base class for the SelectorTableControlRow control on the WPO_Doc_Attach_QuickSelector1 page.
...
' Base class for the SelectorTableControl control on the WPO_Doc_Attach_QuickSelector1 page.
    ...
    ' Setup default pagination settings.
        ...
        Public Overridable Function GetColumnListFromURLParameters(ByVal includeIndexFieldParameter As Boolean, ByVal includeFormulaAndDFKAParameter As Boolean) As ColumnList
            ...
            If includeIndexFieldParameter AndAlso field <> "" Then
                Dim col As BaseColumn = WPO_Doc_Attach1Table.GetColumnByName(field)
                If col IsNot Nothing  AndAlso Not columnList.Contains(col) Then columnList.Add(col)             
            End If
            If includeFormulaAndDFKAParameter Then
                If formula <> "" Then
                    For Each col As BaseColumn In MiscUtils.GetFormulaFieldList(formula, WPO_Doc_Attach1Table.Instance.TableDefinition)
                        If col IsNot Nothing AndAlso Not columnList.Contains(col) Then columnList.Add(col)             
                    Next
                
                ElseIf displayFieldName <> ""
                    Dim col As BaseColumn = WPO_Doc_Attach1Table.GetColumnByName(displayFieldName)
                    If col IsNot Nothing AndAlso Not columnList.Contains(col) Then columnList.Add(col)             
                End If
            End If
            Return columnList            
      ...
      Next
            ...
            End If
		        ...
		        If Not String.IsNullOrEmpty(Me.Page.Request("dependOnField")) AndAlso Me.Page.Request("dependOnVal") IsNot Nothing Then
			          Dim dependOnCol as BaseColumn = WPO_Doc_Attach1Table.GetColumnByName(Me.Page.GetDecryptedURLParameter("dependOnField"))
    ...
    End Function
      ...
      Next
        ...
        Public Overridable Function GetAutoCompletionList_Search(ByVal prefixText As String, ByVal count As Integer) As String()
            ...
            Dim recordList () As ePortalWFApproval.Business.WPO_Doc_Attach1Record = WPO_Doc_Attach1Table.GetRecords(filterJoin, wc, Nothing, 0, count,count)
            Dim rec As WPO_Doc_Attach1Record = Nothing
