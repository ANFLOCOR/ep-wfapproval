...
' WPR_Line_QuickSelector.aspx page.  The Row or RecordControl classes are the 
...
Namespace ePortalWFApproval.UI.Controls.WPR_Line_QuickSelector
...
' Base class for the SelectorTableControlRow control on the WPR_Line_QuickSelector page.
...
' Base class for the SelectorTableControl control on the WPR_Line_QuickSelector page.
    ...
    ' Setup default pagination settings.
        ...
        Public Overridable Function GetColumnListFromURLParameters(ByVal includeIndexFieldParameter As Boolean, ByVal includeFormulaAndDFKAParameter As Boolean) As ColumnList
            ...
            If includeIndexFieldParameter AndAlso field <> "" Then
                Dim col As BaseColumn = WPR_LineTable.GetColumnByName(field)
                If col IsNot Nothing  AndAlso Not columnList.Contains(col) Then columnList.Add(col)             
            End If
            If includeFormulaAndDFKAParameter Then
                If formula <> "" Then
                    For Each col As BaseColumn In MiscUtils.GetFormulaFieldList(formula, WPR_LineTable.Instance.TableDefinition)
                        If col IsNot Nothing AndAlso Not columnList.Contains(col) Then columnList.Add(col)             
                    Next
                
                ElseIf displayFieldName <> ""
                    Dim col As BaseColumn = WPR_LineTable.GetColumnByName(displayFieldName)
                    If col IsNot Nothing AndAlso Not columnList.Contains(col) Then columnList.Add(col)             
                End If
            End If
            Return columnList            
      ...
      Next
            ...
            End If
		        ...
		        If Not String.IsNullOrEmpty(Me.Page.Request("dependOnField")) AndAlso Me.Page.Request("dependOnVal") IsNot Nothing Then
			          Dim dependOnCol as BaseColumn = WPR_LineTable.GetColumnByName(Me.Page.GetDecryptedURLParameter("dependOnField"))
    ...
    End Function
      ...
      Next
        ...
        Public Overridable Function GetAutoCompletionList_Search(ByVal prefixText As String, ByVal count As Integer) As String()
            ...
            Dim recordList () As ePortalWFApproval.Business.WPR_LineRecord = WPR_LineTable.GetRecords(filterJoin, wc, Nothing, 0, count,count)
            Dim rec As WPR_LineRecord = Nothing
