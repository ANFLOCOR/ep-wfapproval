if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalSel_WASS_User_Dynamics_Company1DrillDown') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalSel_WASS_User_Dynamics_Company1DrillDown 
GO
...
CREATE PROCEDURE pePortalWFApprovalSel_WASS_User_Dynamics_Company1DrillDown
...
BEGIN
    SET NOCOUNT ON
        ...
        SET @l_temp_col_type_query = N'SELECT @data_type_out = DATA_TYPE, @precision_out = NUMERIC_PRECISION, @scale_out = NUMERIC_SCALE, @max_out = CHARACTER_MAXIMUM_LENGTH FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME=''sel_WASS_User_Dynamics_Company'' and COLUMN_NAME = ' + '''' + @l_temp_col_name + ''''
    ...
    BEGIN
        ...
        SET @l_query_from = 
            ...
            'AS [Sel_WASS_User_Dyna_1664604136_]' +
