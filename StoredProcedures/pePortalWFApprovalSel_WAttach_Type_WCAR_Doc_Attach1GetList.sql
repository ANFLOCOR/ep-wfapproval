if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalSel_WAttach_Type_WCAR_Doc_Attach1GetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalSel_WAttach_Type_WCAR_Doc_Attach1GetList 
GO
...
-- Returns a query resultset from table [dbo].[sel_WAttach_Type_WCAR_Doc_Attach]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalSel_WAttach_Type_WCAR_Doc_Attach1GetList
...
BEGIN
    ...
    BEGIN
        ...
        SET @l_query_select = 'WITH sel_WAttach_Type_WCAR_Doc_Attach_ AS ( SELECT  '
        SET @l_query_rownum = 'ROW_NUMBER() OVER(' + @l_sort_str + ') AS IS_ROWNUM_COL,'
        SET @l_query_cols = 
            N'sel_WAttach_Type_WCAR_Doc_Attach_.[WCDA_ID],
            sel_WAttach_Type_WCAR_Doc_Attach_.[WCDA_WCD_ID],
            sel_WAttach_Type_WCAR_Doc_Attach_.[WCDA_Desc],
