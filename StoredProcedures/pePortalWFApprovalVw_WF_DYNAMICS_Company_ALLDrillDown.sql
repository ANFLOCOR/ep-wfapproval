if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalVw_WF_DYNAMICS_Company_ALLDrillDown') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalVw_WF_DYNAMICS_Company_ALLDrillDown 
GO
...
CREATE PROCEDURE pePortalWFApprovalVw_WF_DYNAMICS_Company_ALLDrillDown
...
BEGIN
    SET NOCOUNT ON
        ...
        SET @l_temp_col_type_query = N'SELECT @data_type_out = DATA_TYPE, @precision_out = NUMERIC_PRECISION, @scale_out = NUMERIC_SCALE, @max_out = CHARACTER_MAXIMUM_LENGTH FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME=''vw_WF_DYNAMICS_Company_ALL'' and COLUMN_NAME = ' + '''' + @l_temp_col_name + ''''
    ...
    SET @l_from_str = '[dbo].[vw_WF_DYNAMICS_Company_ALL] vw_WF_DYNAMICS_Company_ALL_'
    ...
    BEGIN
        ...
        SET @l_query_from = 
            ...
            'AS [vw_WF_DYNAMICS_Company_ALL_]' +
    ...
    BEGIN
        ...
        SET @l_query_select = 'SELECT '
        SET @l_query_from = 
            ' FROM [dbo].[vw_WF_DYNAMICS_Company_ALL] vw_WF_DYNAMICS_Company_ALL_ ' + 
            'WHERE 1=2;'
