if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalWCanvass_PO_Map_Temp1GetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalWCanvass_PO_Map_Temp1GetList 
GO
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalWCanvass_PO_Map_Temp1GetList
...
BEGIN
    ...
    BEGIN
        ...
        SET @l_query_from = 
            ' FROM ( ' +
                N'SELECT TOP 100 PERCENT IS_ROWNUM_COL, [Temp_ID] from @IS_TEMP_T_GETLIST ' +
                'WHERE IS_ROWNUM_COL >= '+ convert(varchar, @l_start_gen_row_num) + 
                ') IS_ALIAS LEFT JOIN ' +
                @l_from_str + ' ';

        SET @l_query_where = 
            N'ON WCanvass_PO_Map_Temp_.[Temp_ID] = IS_ALIAS.[Temp_ID] ' 
