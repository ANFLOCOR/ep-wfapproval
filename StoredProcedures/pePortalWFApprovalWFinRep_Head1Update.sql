if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalWFinRep_Head1Update') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalWFinRep_Head1Update 
GO
...
CREATE PROCEDURE pePortalWFApprovalWFinRep_Head1Update
    @pk_HFIN_ID int,
    @p_HFIN_DT_ID int,
...
-- Check whether the record still exists before doing update
    ...
    ELSE
        BEGIN
            ...
            Select @l_newValue = CAST(BINARY_CHECKSUM([HFIN_ID],[HFIN_DT_ID],[HFIN_Code],[HFIN_Submit],[HFIN_Status],[HFIN_C_ID],[HFIN_U_ID],[HFIN_Remark],[HFIN_PRTCNT],[HFIN_FinID],[HFIN_Month],[HFIN_Year],[HFIN_RptCount],[HFIN_Description],[HFIN_File]) AS nvarchar(max)) 
