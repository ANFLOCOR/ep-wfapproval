if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalWFinRepNGP_Attachment1GetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalWFinRepNGP_Attachment1GetList 
GO
...
-- Returns a query resultset from table [dbo].[WFinRepNGP_Attachment]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalWFinRepNGP_Attachment1GetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[WFinRepNGP_Attachment] WFinRepNGP_Attachment_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY WFinRepNGP_Attachment_.[WFRNGPAt_ID] asc '
        ...
        SET @l_query_select = 'WITH WFinRepNGP_Attachment_ AS ( SELECT  '
        SET @l_query_rownum = 'ROW_NUMBER() OVER(' + @l_sort_str + ') AS IS_ROWNUM_COL,'
        SET @l_query_cols = 
            N'WFinRepNGP_Attachment_.[WFRNGPAt_ID],
            WFinRepNGP_Attachment_.[WFRNGPAt_WFRCHNGP_ID],
            WFinRepNGP_Attachment_.[WFRNGPAt_Desc],
            WFinRepNGP_Attachment_.[WFRNGPAt_Doc],
            CAST(BINARY_CHECKSUM(WFinRepNGP_Attachment_.[WFRNGPAt_ID],WFinRepNGP_Attachment_.[WFRNGPAt_WFRCHNGP_ID],WFinRepNGP_Attachment_.[WFRNGPAt_Desc],WFinRepNGP_Attachment_.[WFRNGPAt_Doc]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = ' FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM WFinRepNGP_Attachment_ '
    ...
    BEGIN
        ...
        SET @l_query_cols = 
            ...
            CAST(BINARY_CHECKSUM(WFinRepNGP_Attachment_.[WFRNGPAt_ID],WFinRepNGP_Attachment_.[WFRNGPAt_WFRCHNGP_ID],WFinRepNGP_Attachment_.[WFRNGPAt_Desc],WFinRepNGP_Attachment_.[WFRNGPAt_Doc]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = 
            ' FROM [dbo].[WFinRepNGP_Attachment] WFinRepNGP_Attachment_ ' + 
            'WHERE 1=2;'
