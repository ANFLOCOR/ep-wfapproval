if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalWCanvass_Internal1Export') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalWCanvass_Internal1Export 
GO
...
CREATE PROCEDURE pePortalWFApprovalWCanvass_Internal1Export
    ...
    BEGIN
        ...
        IF @p_select_str IS NULL
            BEGIN
            SET @l_select_str = 
                N'IsNULL(Convert(nvarchar, WCanvass_Internal_.[WCI_ID]), '''') + ''' + @p_separator_str + ''' +' +
                N'IsNULL(Convert(nvarchar, WCanvass_Internal_.[WCI_C_ID]), '''') + ''' + @p_separator_str + ''' +' +
                N'IsNULL(Convert(nvarchar, WCanvass_Internal_.[WCI_WPRD_ID]), '''') + ''' + @p_separator_str + ''' +' +
                N'IsNULL(Convert(nvarchar, WCanvass_Internal_.[WCI_Date], 21), '''') + ''' + @p_separator_str + ''' +' +
                N'N''"'' + IsNULL(Convert(nvarchar, WCanvass_Internal_.[WCI_Submit]), '''') + N''"'' + ''' + @p_separator_str + ''' +' +
                N'N''"'' + REPLACE(IsNULL(WCanvass_Internal_.[WCI_Status], ''''), N''"'', N''""'') + N''"''  + ''' + @p_separator_str + ''' +' +
