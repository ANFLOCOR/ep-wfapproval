if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalCategoryGetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalCategoryGetList 
GO
...
-- Returns a query resultset from table [dbo].[Category]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalCategoryGetList
...
BEGIN
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY Category_.[ID] asc '
        ...
        SET @l_query_select = 'WITH Category_ AS ( SELECT  '
        SET @l_query_rownum = 'ROW_NUMBER() OVER(' + @l_sort_str + ') AS IS_ROWNUM_COL,'
        SET @l_query_cols = 
            N'Category_.[ID],
            Category_.[Accatnum],
            Category_.[Accatdesc],
            Category_.[TypicalBalance],
            Category_.[Orderno],
            Category_.[Orderno1],
            Category_.[ReportID],
            Category_.[LevelNo],
            Category_.[Level1],
            Category_.[Level1_Ord],
            Category_.[Level2],
            Category_.[Level2_Ord],
            Category_.[Indent],
            Category_.[CreatedBy],
            Category_.[DateCreated],
            Category_.[ModifiedBy],
            Category_.[DateModified],
            Category_.[IsPrimary],
            Category_.[TotalOf],
            CAST(BINARY_CHECKSUM(Category_.[ID],Category_.[Accatnum],Category_.[Accatdesc],Category_.[TypicalBalance],Category_.[Orderno],Category_.[Orderno1],Category_.[ReportID],Category_.[LevelNo],Category_.[Level1],Category_.[Level1_Ord],Category_.[Level2],Category_.[Level2_Ord],Category_.[Indent],Category_.[CreatedBy],Category_.[DateCreated],Category_.[ModifiedBy],Category_.[DateModified],Category_.[IsPrimary],Category_.[TotalOf]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = ' FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM Category_ '
