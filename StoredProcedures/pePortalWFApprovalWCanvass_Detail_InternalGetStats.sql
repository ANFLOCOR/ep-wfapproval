if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalWCanvass_Detail_InternalGetStats') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalWCanvass_Detail_InternalGetStats 
GO
...
-- sum, avg, max, etc
CREATE PROCEDURE pePortalWFApprovalWCanvass_Detail_InternalGetStats
    ...
    BEGIN
        ...
        SET @l_temp_col_type_query = N'SELECT @data_type_out = DATA_TYPE, @precision_out = NUMERIC_PRECISION, @scale_out = NUMERIC_SCALE FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME=''WCanvass_Detail_Internal'' and COLUMN_NAME = ' + '''' + @l_temp_col_name + ''''
        ...
        BEGIN
            SET @l_query = 
                'SELECT count(*) from ' + '[dbo].[WCanvass_Detail_Internal] WCanvass_Detail_Internal_ ' +
                'WHERE 1=2;'
            EXECUTE (@l_query)
        END
        SET NOCOUNT OFF
