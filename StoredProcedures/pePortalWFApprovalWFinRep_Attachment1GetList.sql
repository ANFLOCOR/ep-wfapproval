if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalWFinRep_Attachment1GetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalWFinRep_Attachment1GetList 
GO
...
-- Returns a query resultset from table [dbo].[WFinRep_Attachment]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalWFinRep_Attachment1GetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[WFinRep_Attachment] WFinRep_Attachment_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY WFinRep_Attachment_.[WFRA_ID] asc '
        ...
        SET @l_query_select = 'WITH WFinRep_Attachment_ AS ( SELECT  '
        SET @l_query_rownum = 'ROW_NUMBER() OVER(' + @l_sort_str + ') AS IS_ROWNUM_COL,'
        SET @l_query_cols = 
            N'WFinRep_Attachment_.[WFRA_ID],
            WFinRep_Attachment_.[WFRA_FIN_ID],
            WFinRep_Attachment_.[WFRA_Desc],
            WFinRep_Attachment_.[WFRA_Doc],
            CAST(BINARY_CHECKSUM(WFinRep_Attachment_.[WFRA_ID],WFinRep_Attachment_.[WFRA_FIN_ID],WFinRep_Attachment_.[WFRA_Desc],WFinRep_Attachment_.[WFRA_Doc]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = ' FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM WFinRep_Attachment_ '
    ...
    BEGIN
        ...
        SET @l_query_cols = 
            ...
            CAST(BINARY_CHECKSUM(WFinRep_Attachment_.[WFRA_ID],WFinRep_Attachment_.[WFRA_FIN_ID],WFinRep_Attachment_.[WFRA_Desc],WFinRep_Attachment_.[WFRA_Doc]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = 
            ' FROM [dbo].[WFinRep_Attachment] WFinRep_Attachment_ ' + 
            'WHERE 1=2;'
