if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalSel_FIN_WASP_Report1GetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalSel_FIN_WASP_Report1GetList 
GO
...
-- Returns a query resultset from table [dbo].[sel_FIN_WASP_Report]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalSel_FIN_WASP_Report1GetList
...
BEGIN
    ...
    BEGIN
        ...
        SET @l_query_select = 'WITH sel_FIN_WASP_Report_ AS ( SELECT  '
        SET @l_query_rownum = 'ROW_NUMBER() OVER(' + @l_sort_str + ') AS IS_ROWNUM_COL,'
        SET @l_query_cols = 
            N'sel_FIN_WASP_Report_.[W_Rpt_ID],
            sel_FIN_WASP_Report_.[W_Rpt_Display_Name],
            sel_FIN_WASP_Report_.[W_Rpt_Desc],
            sel_FIN_WASP_Report_.[W_Rpt_Server_Path],
            sel_FIN_WASP_Report_.[W_Rpt_IsActive],
            sel_FIN_WASP_Report_.[W_Rpt_A_ID],
            sel_FIN_WASP_Report_.[W_Rpt_ShortName],
            sel_FIN_WASP_Report_.[W_Rpt_CompanyID],
            sel_FIN_WASP_Report_.[W_Rpt_SortOrder]'

        SET @l_query_from = 'FROM ' + @l_from_str + ' ' + @l_join_str + + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM sel_FIN_WASP_Report_ '
    ...
    BEGIN
        ...
        SET @l_query_cols = 
            ...
            sel_FIN_WASP_Report_.[W_Rpt_SortOrder]'
        SET @l_query_from = 
            ' FROM [dbo].[sel_FIN_WASP_Report] sel_FIN_WASP_Report_ ' + 
            'WHERE 1=2;'
