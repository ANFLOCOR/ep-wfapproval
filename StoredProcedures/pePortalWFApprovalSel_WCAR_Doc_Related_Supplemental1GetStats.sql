if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalSel_WCAR_Doc_Related_Supplemental1GetStats') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalSel_WCAR_Doc_Related_Supplemental1GetStats 
GO
...
-- sum, avg, max, etc
CREATE PROCEDURE pePortalWFApprovalSel_WCAR_Doc_Related_Supplemental1GetStats
    ...
    BEGIN
        ...
        SET @l_temp_col_type_query = N'SELECT @data_type_out = DATA_TYPE, @precision_out = NUMERIC_PRECISION, @scale_out = NUMERIC_SCALE FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME=''sel_WCAR_Doc_Related_Supplemental'' and COLUMN_NAME = ' + '''' + @l_temp_col_name + ''''
        ...
        BEGIN
            ...
            SET @l_query = 
                ...
                ') ' + 'sel_WCAR_Doc_Related_Supplemental_'
        ...
        BEGIN
            SET @l_query = 
                'SELECT count(*) from ' + '[dbo].[sel_WCAR_Doc_Related_Supplemental] sel_WCAR_Doc_Related_Supplemental_ ' +
                'WHERE 1=2;'
            EXECUTE (@l_query)
        END
        SET NOCOUNT OFF
