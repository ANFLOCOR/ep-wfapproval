if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalWFinRepNGP_Activity1DrillDown') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalWFinRepNGP_Activity1DrillDown 
GO
...
CREATE PROCEDURE pePortalWFApprovalWFinRepNGP_Activity1DrillDown
...
BEGIN
    SET NOCOUNT ON
        ...
        SET @l_temp_col_type_query = N'SELECT @data_type_out = DATA_TYPE, @precision_out = NUMERIC_PRECISION, @scale_out = NUMERIC_SCALE, @max_out = CHARACTER_MAXIMUM_LENGTH FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME=''WFinRepNGP_Activity'' and COLUMN_NAME = ' + '''' + @l_temp_col_name + ''''
