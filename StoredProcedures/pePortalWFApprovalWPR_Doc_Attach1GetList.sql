if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalWPR_Doc_Attach1GetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalWPR_Doc_Attach1GetList 
GO
...
-- Returns a query resultset from table [dbo].[WPR_Doc_Attach]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalWPR_Doc_Attach1GetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[WPR_Doc_Attach] WPR_Doc_Attach_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY WPR_Doc_Attach_.[WPRDA_ID] asc '
        ...
        SET @l_query_cols = 
            N'WPR_Doc_Attach_.[WPRDA_ID],
            WPR_Doc_Attach_.[WPRDA_WPRD_ID],
            WPR_Doc_Attach_.[WPRDA_Desc],
            WPR_Doc_Attach_.[WPRDA_File],
            WPR_Doc_Attach_.[WPRDA_WAT_ID],
            CAST(BINARY_CHECKSUM(WPR_Doc_Attach_.[WPRDA_ID],WPR_Doc_Attach_.[WPRDA_WPRD_ID],WPR_Doc_Attach_.[WPRDA_Desc],WPR_Doc_Attach_.[WPRDA_File],WPR_Doc_Attach_.[WPRDA_WAT_ID]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = 
            ' FROM ( ' +
                N'SELECT TOP 100 PERCENT IS_ROWNUM_COL, [WPRDA_ID] from @IS_TEMP_T_GETLIST ' +
                'WHERE IS_ROWNUM_COL >= '+ convert(varchar, @l_start_gen_row_num) + 
                ') IS_ALIAS LEFT JOIN ' +
                @l_from_str + ' ';

        SET @l_query_where = 
            N'ON WPR_Doc_Attach_.[WPRDA_ID] = IS_ALIAS.[WPRDA_ID] ' 
