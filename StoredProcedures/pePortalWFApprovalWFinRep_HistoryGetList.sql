if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalWFinRep_HistoryGetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalWFinRep_HistoryGetList 
GO
...
-- Returns a query resultset from table [dbo].[WFinRep_History]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalWFinRep_HistoryGetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[WFinRep_History] WFinRep_History_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY WFinRep_History_.[WFRHi_ID] asc '
        ...
        SET @l_query_select = 'WITH WFinRep_History_ AS ( SELECT  '
        SET @l_query_rownum = 'ROW_NUMBER() OVER(' + @l_sort_str + ') AS IS_ROWNUM_COL,'
        SET @l_query_cols = 
            N'WFinRep_History_.[WFRHi_ID],
            WFinRep_History_.[WFRHi_WFRCH_ID],
            WFinRep_History_.[WFRHi_File],
            WFinRep_History_.[WFRHi_CreatedBy],
            WFinRep_History_.[WFRHi_DateCreated],
            CAST(BINARY_CHECKSUM(WFinRep_History_.[WFRHi_ID],WFinRep_History_.[WFRHi_WFRCH_ID],WFinRep_History_.[WFRHi_File],WFinRep_History_.[WFRHi_CreatedBy],WFinRep_History_.[WFRHi_DateCreated]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = ' FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM WFinRep_History_ '
