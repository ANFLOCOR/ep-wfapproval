if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalWPO_Step1GetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalWPO_Step1GetList 
GO
...
-- Returns a query resultset from table [dbo].[WPO_Step]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalWPO_Step1GetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[WPO_Step] WPO_Step_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY WPO_Step_.[WPO_S_ID] asc '
        ...
        SET @l_query_select = 'WITH WPO_Step_ AS ( SELECT  '
        SET @l_query_rownum = 'ROW_NUMBER() OVER(' + @l_sort_str + ') AS IS_ROWNUM_COL,'
        SET @l_query_cols = 
            N'WPO_Step_.[WPO_S_ID],
            WPO_Step_.[WPO_S_WDT_ID],
            WPO_Step_.[WPO_S_ID_Next],
            WPO_Step_.[WPO_S_Step_Type],
            WPO_Step_.[WPO_S_Desc],
            WPO_Step_.[WPO_S_Approval_Needed],
            WPO_Step_.[WPO_S_C_ID],
            CAST(BINARY_CHECKSUM(WPO_Step_.[WPO_S_ID],WPO_Step_.[WPO_S_WDT_ID],WPO_Step_.[WPO_S_ID_Next],WPO_Step_.[WPO_S_Step_Type],WPO_Step_.[WPO_S_Desc],WPO_Step_.[WPO_S_Approval_Needed],WPO_Step_.[WPO_S_C_ID]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = ' FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM WPO_Step_ '
