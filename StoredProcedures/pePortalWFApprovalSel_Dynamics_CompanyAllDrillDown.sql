if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalSel_Dynamics_CompanyAllDrillDown') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalSel_Dynamics_CompanyAllDrillDown 
GO
...
CREATE PROCEDURE pePortalWFApprovalSel_Dynamics_CompanyAllDrillDown
...
BEGIN
    SET NOCOUNT ON
        ...
        SET @l_temp_col_type_query = N'SELECT @data_type_out = DATA_TYPE, @precision_out = NUMERIC_PRECISION, @scale_out = NUMERIC_SCALE, @max_out = CHARACTER_MAXIMUM_LENGTH FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME=''sel_Dynamics_CompanyAll'' and COLUMN_NAME = ' + '''' + @l_temp_col_name + ''''
    ...
    SET @l_from_str = '[dbo].[sel_Dynamics_CompanyAll] sel_Dynamics_CompanyAll_'
    ...
    BEGIN
        ...
        SET @l_query_select = 'SELECT '
        SET @l_query_from = 
            ' FROM [dbo].[sel_Dynamics_CompanyAll] sel_Dynamics_CompanyAll_ ' + 
            'WHERE 1=2;'
