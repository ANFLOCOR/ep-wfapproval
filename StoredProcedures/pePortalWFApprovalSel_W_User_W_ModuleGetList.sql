if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalSel_W_User_W_ModuleGetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalSel_W_User_W_ModuleGetList 
GO
...
-- Returns a query resultset from table [dbo].[sel_W_User_W_Module]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalSel_W_User_W_ModuleGetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[sel_W_User_W_Module] sel_W_User_W_Module_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY sel_W_User_W_Module_.[W_U_ID],sel_W_User_W_Module_.[W_MS_ID] asc '
        ...
        SET @l_temp_colsWithAlias = 
            N'sel_W_User_W_Module_.[W_U_ID],sel_W_User_W_Module_.[W_MS_ID]'
        SET @l_temp_from = 
            ' FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + 
            @l_where_str + ' ' + 
            @l_sort_str
        ...
        SET @l_query_cols = 
            N'sel_W_User_W_Module_.[W_U_ID],
            sel_W_User_W_Module_.[W_MS_ID],
            CAST(BINARY_CHECKSUM(sel_W_User_W_Module_.[W_U_ID],sel_W_User_W_Module_.[W_MS_ID]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = 
            ' FROM ( ' +
                N'SELECT TOP 100 PERCENT IS_ROWNUM_COL, [W_U_ID],[W_MS_ID] from @IS_TEMP_T_GETLIST ' +
                'WHERE IS_ROWNUM_COL >= '+ convert(varchar, @l_start_gen_row_num) + 
                ') IS_ALIAS LEFT JOIN ' +
                @l_from_str + ' ';

        SET @l_query_where = 
            N'ON sel_W_User_W_Module_.[W_U_ID] = IS_ALIAS.[W_U_ID] AND sel_W_User_W_Module_.[W_MS_ID] = IS_ALIAS.[W_MS_ID] ' 
    ...
    BEGIN
        ...
        SET @l_query_cols = 
            ...
            CAST(BINARY_CHECKSUM(sel_W_User_W_Module_.[W_U_ID],sel_W_User_W_Module_.[W_MS_ID]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = 
            ' FROM [dbo].[sel_W_User_W_Module] sel_W_User_W_Module_ ' + 
            'WHERE 1=2;'
