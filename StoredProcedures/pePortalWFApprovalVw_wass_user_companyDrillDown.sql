if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalVw_wass_user_companyDrillDown') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalVw_wass_user_companyDrillDown 
GO
...
CREATE PROCEDURE pePortalWFApprovalVw_wass_user_companyDrillDown
...
BEGIN
    SET NOCOUNT ON
        ...
        SET @l_temp_col_type_query = N'SELECT @data_type_out = DATA_TYPE, @precision_out = NUMERIC_PRECISION, @scale_out = NUMERIC_SCALE, @max_out = CHARACTER_MAXIMUM_LENGTH FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME=''vw_wass_user_company'' and COLUMN_NAME = ' + '''' + @l_temp_col_name + ''''
