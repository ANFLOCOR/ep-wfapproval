if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalPOP10100_HEADER_ALL2DrillDown') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalPOP10100_HEADER_ALL2DrillDown 
GO
...
CREATE PROCEDURE pePortalWFApprovalPOP10100_HEADER_ALL2DrillDown
...
BEGIN
    SET NOCOUNT ON
        ...
        SET @l_temp_col_type_query = N'SELECT @data_type_out = DATA_TYPE, @precision_out = NUMERIC_PRECISION, @scale_out = NUMERIC_SCALE, @max_out = CHARACTER_MAXIMUM_LENGTH FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME=''POP10100_HEADER_ALL'' and COLUMN_NAME = ' + '''' + @l_temp_col_name + ''''
