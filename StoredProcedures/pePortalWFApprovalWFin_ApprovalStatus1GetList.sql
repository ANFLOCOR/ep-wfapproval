if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalWFin_ApprovalStatus1GetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalWFin_ApprovalStatus1GetList 
GO
...
-- Returns a query resultset from table [dbo].[WFin_ApprovalStatus]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalWFin_ApprovalStatus1GetList
...
BEGIN
    ...
    BEGIN
        ...
        SET @l_query_select = 'WITH WFin_ApprovalStatus_ AS ( SELECT  '
        SET @l_query_rownum = 'ROW_NUMBER() OVER(' + @l_sort_str + ') AS IS_ROWNUM_COL,'
        SET @l_query_cols = 
            N'WFin_ApprovalStatus_.[WPO_STAT_CD],
            WFin_ApprovalStatus_.[WPO_STAT_DESC],
            CAST(BINARY_CHECKSUM(WFin_ApprovalStatus_.[WPO_STAT_CD],WFin_ApprovalStatus_.[WPO_STAT_DESC]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = ' FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM WFin_ApprovalStatus_ '
