if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalWFinRep_DocTypeGet') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalWFinRep_DocTypeGet 
GO
...
CREATE PROCEDURE pePortalWFApprovalWFinRep_DocTypeGet
        @pk_WFIN_DT_ID int
AS
DECLARE
    @l_count int
