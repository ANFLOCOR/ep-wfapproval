if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalSel_WFIN_ApproverPageGetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalSel_WFIN_ApproverPageGetList 
GO
...
-- Returns a query resultset from table [dbo].[sel_WFIN_ApproverPage]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalSel_WFIN_ApproverPageGetList
...
BEGIN
    ...
    BEGIN
        ...
        SET @l_query_select = 'WITH sel_WFIN_ApproverPage_ AS ( SELECT  '
        SET @l_query_rownum = 'ROW_NUMBER() OVER(' + @l_sort_str + ') AS IS_ROWNUM_COL,'
        SET @l_query_cols = 
            N'sel_WFIN_ApproverPage_.[AFIN_ID],
            sel_WFIN_ApproverPage_.[AFIN_WS_ID],
            sel_WFIN_ApproverPage_.[AFIN_WSD_ID],
            sel_WFIN_ApproverPage_.[AFIN_WDT_ID],
            sel_WFIN_ApproverPage_.[AFIN_W_U_ID],
            sel_WFIN_ApproverPage_.[AFIN_Status],
            sel_WFIN_ApproverPage_.[AFIN_Date_Assign],
            sel_WFIN_ApproverPage_.[AFIN_Date_Action],
            sel_WFIN_ApproverPage_.[AFIN_Remark],
            sel_WFIN_ApproverPage_.[AFIN_Is_Done],
            sel_WFIN_ApproverPage_.[AFIN_FinID],
            sel_WFIN_ApproverPage_.[HFIN_C_ID],
            sel_WFIN_ApproverPage_.[FIN_Year],
            sel_WFIN_ApproverPage_.[FIN_Month],
            sel_WFIN_ApproverPage_.[FIn_Description],
            sel_WFIN_ApproverPage_.[AFIN_HFIN_ID]'
