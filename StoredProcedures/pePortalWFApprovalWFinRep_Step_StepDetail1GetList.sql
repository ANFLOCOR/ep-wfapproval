if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalWFinRep_Step_StepDetail1GetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalWFinRep_Step_StepDetail1GetList 
GO
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalWFinRep_Step_StepDetail1GetList
...
BEGIN
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY WFinRep_Step_StepDetail_.[WFIN_SD_ID] asc '
        ...
        SET @l_temp_colsWithAlias = 
            N'WFinRep_Step_StepDetail_.[WFIN_SD_ID]'
        SET @l_temp_from = 
            ' FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + 
            @l_where_str + ' ' + 
            @l_sort_str
        ...
        SET @l_query_cols = 
            ...
            CAST(BINARY_CHECKSUM(WFinRep_Step_StepDetail_.[WFIN_S_ID],WFinRep_Step_StepDetail_.[WFIN_S_WDT_ID],WFinRep_Step_StepDetail_.[WFIN_S_ID_Next],WFinRep_Step_StepDetail_.[WFIN_S_Step_Type],WFinRep_Step_StepDetail_.[WFIN_S_Approval_Needed],WFinRep_Step_StepDetail_.[WFIN_SD_ID],WFinRep_Step_StepDetail_.[WFIN_SD_W_U_ID],WFinRep_Step_StepDetail_.[W_U_Full_Name]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = 
            ' FROM ( ' +
                N'SELECT TOP 100 PERCENT IS_ROWNUM_COL, [WFIN_SD_ID] from @IS_TEMP_T_GETLIST ' +
                'WHERE IS_ROWNUM_COL >= '+ convert(varchar, @l_start_gen_row_num) + 
                ') IS_ALIAS LEFT JOIN ' +
                @l_from_str + ' ';

        SET @l_query_where = 
            N'ON WFinRep_Step_StepDetail_.[WFIN_SD_ID] = IS_ALIAS.[WFIN_SD_ID] ' 
