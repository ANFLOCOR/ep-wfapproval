if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalVw_Trx_Summary_GroupByCompanyYearPeriodGetStats') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalVw_Trx_Summary_GroupByCompanyYearPeriodGetStats 
GO
...
-- sum, avg, max, etc
CREATE PROCEDURE pePortalWFApprovalVw_Trx_Summary_GroupByCompanyYearPeriodGetStats
    ...
    BEGIN
        ...
        SET @l_temp_col_type_query = N'SELECT @data_type_out = DATA_TYPE, @precision_out = NUMERIC_PRECISION, @scale_out = NUMERIC_SCALE FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME=''vw_Trx_Summary_GroupByCompanyYearPeriod'' and COLUMN_NAME = ' + '''' + @l_temp_col_name + ''''
        ...
        BEGIN
            ...
            SET @l_query = 
                ...
                ') ' + 'vw_Trx_Summary_GroupByCompanyYearPeriod_'
        ...
        BEGIN
            SET @l_query = 
                'SELECT count(*) from ' + '[dbo].[vw_Trx_Summary_GroupByCompanyYearPeriod] vw_Trx_Summary_GroupByCompanyYearPeriod_ ' +
                'WHERE 1=2;'
            EXECUTE (@l_query)
        END
        SET NOCOUNT OFF
