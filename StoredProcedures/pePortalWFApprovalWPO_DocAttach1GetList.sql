if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalWPO_DocAttach1GetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalWPO_DocAttach1GetList 
GO
...
-- Returns a query resultset from table [dbo].[WPO_DocAttach]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalWPO_DocAttach1GetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[WPO_DocAttach] WPO_DocAttach_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY WPO_DocAttach_.[WPODA_ID] asc '
        ...
        SET @l_query_cols = 
            N'WPO_DocAttach_.[WPODA_ID],
            WPO_DocAttach_.[WPODA_PONum],
            WPO_DocAttach_.[WPODA_Desc],
            WPO_DocAttach_.[WPODA_File],
            WPO_DocAttach_.[WPODA_WAT_ID],
            CAST(BINARY_CHECKSUM(WPO_DocAttach_.[WPODA_ID],WPO_DocAttach_.[WPODA_PONum],WPO_DocAttach_.[WPODA_Desc],WPO_DocAttach_.[WPODA_File],WPO_DocAttach_.[WPODA_WAT_ID]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = 
            ' FROM ( ' +
                N'SELECT TOP 100 PERCENT IS_ROWNUM_COL, [WPODA_ID] from @IS_TEMP_T_GETLIST ' +
                'WHERE IS_ROWNUM_COL >= '+ convert(varchar, @l_start_gen_row_num) + 
                ') IS_ALIAS LEFT JOIN ' +
                @l_from_str + ' ';

        SET @l_query_where = 
            N'ON WPO_DocAttach_.[WPODA_ID] = IS_ALIAS.[WPODA_ID] ' 
    ...
    BEGIN
        ...
        SET @l_query_cols = 
            ...
            CAST(BINARY_CHECKSUM(WPO_DocAttach_.[WPODA_ID],WPO_DocAttach_.[WPODA_PONum],WPO_DocAttach_.[WPODA_Desc],WPO_DocAttach_.[WPODA_File],WPO_DocAttach_.[WPODA_WAT_ID]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = 
            ' FROM [dbo].[WPO_DocAttach] WPO_DocAttach_ ' + 
            'WHERE 1=2;'
