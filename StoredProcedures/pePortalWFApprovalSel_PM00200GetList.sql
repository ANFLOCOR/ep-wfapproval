if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalSel_PM00200GetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalSel_PM00200GetList 
GO
...
-- Returns a query resultset from table [dbo].[sel_PM00200]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalSel_PM00200GetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[sel_PM00200] sel_PM00200_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY sel_PM00200_.[VENDORID],sel_PM00200_.[Company_ID] asc '
        ...
        IS_ROWNUM_COL int identity(1,1),
                [VENDORID] char(15) COLLATE SQL_Latin1_General_CP1_CI_AS,    [Company_ID] int
        ); '
        ...
        SET @l_temp_cols = 
            N'[VENDORID],[Company_ID]'
        SET @l_temp_select = 
            ') ' + 
            'SELECT ' + 
            'TOP ' + convert(varchar, @l_end_gen_row_num) + ' '
        SET @l_temp_colsWithAlias = 
            N'sel_PM00200_.[VENDORID],sel_PM00200_.[Company_ID]'
        SET @l_temp_from = 
            ' FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + 
            @l_where_str + ' ' + 
            @l_sort_str
        ...
        SET @l_query_cols = 
            N'sel_PM00200_.[VENDORID],
            sel_PM00200_.[VENDNAME],
            sel_PM00200_.[Company_ID],
            CAST(BINARY_CHECKSUM(sel_PM00200_.[VENDORID],sel_PM00200_.[VENDNAME],sel_PM00200_.[Company_ID]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = 
            ' FROM ( ' +
                N'SELECT TOP 100 PERCENT IS_ROWNUM_COL, [VENDORID],[Company_ID] from @IS_TEMP_T_GETLIST ' +
                'WHERE IS_ROWNUM_COL >= '+ convert(varchar, @l_start_gen_row_num) + 
                ') IS_ALIAS LEFT JOIN ' +
                @l_from_str + ' ';

        SET @l_query_where = 
            N'ON sel_PM00200_.[VENDORID] = IS_ALIAS.[VENDORID] AND sel_PM00200_.[Company_ID] = IS_ALIAS.[Company_ID] ' 
    ...
    BEGIN
        ...
        SET @l_query_cols = 
            ...
            CAST(BINARY_CHECKSUM(sel_PM00200_.[VENDORID],sel_PM00200_.[VENDNAME],sel_PM00200_.[Company_ID]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = 
            ' FROM [dbo].[sel_PM00200] sel_PM00200_ ' + 
            'WHERE 1=2;'
