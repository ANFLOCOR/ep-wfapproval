if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalSel_SysSetupUsers_Email1GetStats') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalSel_SysSetupUsers_Email1GetStats 
GO
...
-- sum, avg, max, etc
CREATE PROCEDURE pePortalWFApprovalSel_SysSetupUsers_Email1GetStats
    ...
    BEGIN
        ...
        SET @l_temp_col_type_query = N'SELECT @data_type_out = DATA_TYPE, @precision_out = NUMERIC_PRECISION, @scale_out = NUMERIC_SCALE FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME=''sel_SysSetupUsers_Email'' and COLUMN_NAME = ' + '''' + @l_temp_col_name + ''''
        ...
        BEGIN
            SET @l_query = 
                'SELECT count(*) from ' + '[dbo].[sel_SysSetupUsers_Email] sel_SysSetupUsers_Email_ ' +
                'WHERE 1=2;'
            EXECUTE (@l_query)
        END
        SET NOCOUNT OFF
