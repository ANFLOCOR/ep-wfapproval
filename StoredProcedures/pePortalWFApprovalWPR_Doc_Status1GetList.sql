if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalWPR_Doc_Status1GetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalWPR_Doc_Status1GetList 
GO
...
-- Returns a query resultset from table [dbo].[WPR_Doc_Status]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalWPR_Doc_Status1GetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[WPR_Doc_Status] WPR_Doc_Status_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY WPR_Doc_Status_.[WPRDS_ID] asc '
        ...
        SET @l_query_select = 'WITH WPR_Doc_Status_ AS ( SELECT  '
        SET @l_query_rownum = 'ROW_NUMBER() OVER(' + @l_sort_str + ') AS IS_ROWNUM_COL,'
        SET @l_query_cols = 
            N'WPR_Doc_Status_.[WPRDS_ID],
            WPR_Doc_Status_.[WPRDS_Desc],
            WPR_Doc_Status_.[WPRDS_Value],
            CAST(BINARY_CHECKSUM(WPR_Doc_Status_.[WPRDS_ID],WPR_Doc_Status_.[WPRDS_Desc],WPR_Doc_Status_.[WPRDS_Value]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = ' FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM WPR_Doc_Status_ '
    ...
    BEGIN
        ...
        SET @l_query_cols = 
            ...
            CAST(BINARY_CHECKSUM(WPR_Doc_Status_.[WPRDS_ID],WPR_Doc_Status_.[WPRDS_Desc],WPR_Doc_Status_.[WPRDS_Value]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = 
            ' FROM [dbo].[WPR_Doc_Status] WPR_Doc_Status_ ' + 
            'WHERE 1=2;'
