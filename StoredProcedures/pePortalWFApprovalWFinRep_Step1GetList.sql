if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalWFinRep_Step1GetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalWFinRep_Step1GetList 
GO
...
-- Returns a query resultset from table [dbo].[WFinRep_Step]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalWFinRep_Step1GetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[WFinRep_Step] WFinRep_Step_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY WFinRep_Step_.[WFIN_S_ID] asc '
        ...
        SET @l_query_select = 'WITH WFinRep_Step_ AS ( SELECT  '
        SET @l_query_rownum = 'ROW_NUMBER() OVER(' + @l_sort_str + ') AS IS_ROWNUM_COL,'
        SET @l_query_cols = 
            N'WFinRep_Step_.[WFIN_S_ID],
            WFinRep_Step_.[WFIN_S_WDT_ID],
            WFinRep_Step_.[WFIN_S_ID_Next],
            WFinRep_Step_.[WFIN_S_Step_Type],
            WFinRep_Step_.[WFIN_S_Desc],
            WFinRep_Step_.[WFIN_S_Approval_Needed],
            WFinRep_Step_.[WFIN_S_C_ID],
            CAST(BINARY_CHECKSUM(WFinRep_Step_.[WFIN_S_ID],WFinRep_Step_.[WFIN_S_WDT_ID],WFinRep_Step_.[WFIN_S_ID_Next],WFinRep_Step_.[WFIN_S_Step_Type],WFinRep_Step_.[WFIN_S_Desc],WFinRep_Step_.[WFIN_S_Approval_Needed],WFinRep_Step_.[WFIN_S_C_ID]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = ' FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM WFinRep_Step_ '
