if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalVw_Trx_Summary_GroupByCompanyYearPeriodGetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalVw_Trx_Summary_GroupByCompanyYearPeriodGetList 
GO
...
-- Returns a query resultset from table [dbo].[vw_Trx_Summary_GroupByCompanyYearPeriod]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalVw_Trx_Summary_GroupByCompanyYearPeriodGetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[vw_Trx_Summary_GroupByCompanyYearPeriod] vw_Trx_Summary_GroupByCompanyYearPeriod_'
    ...
    BEGIN
        ...
        SET @l_query_select = 'WITH vw_Trx_Summary_GroupByCompanyYearPeriod_ AS ( SELECT  '
        SET @l_query_rownum = 'ROW_NUMBER() OVER(' + @l_sort_str + ') AS IS_ROWNUM_COL,'
        SET @l_query_cols = 
            N'vw_Trx_Summary_GroupByCompanyYearPeriod_.[DynamicsCompanyID],
            vw_Trx_Summary_GroupByCompanyYearPeriod_.[CompanyID],
            vw_Trx_Summary_GroupByCompanyYearPeriod_.[ShortName],
            vw_Trx_Summary_GroupByCompanyYearPeriod_.[GroupID],
            vw_Trx_Summary_GroupByCompanyYearPeriod_.[Year],
            vw_Trx_Summary_GroupByCompanyYearPeriod_.[Period]'

        SET @l_query_from = 'FROM ' + @l_from_str + ' ' + @l_join_str + + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM vw_Trx_Summary_GroupByCompanyYearPeriod_ '
    ...
    BEGIN
        ...
        SET @l_query_cols = 
            ...
            vw_Trx_Summary_GroupByCompanyYearPeriod_.[Period]'
        SET @l_query_from = 
            ' FROM [dbo].[vw_Trx_Summary_GroupByCompanyYearPeriod] vw_Trx_Summary_GroupByCompanyYearPeriod_ ' + 
            'WHERE 1=2;'
