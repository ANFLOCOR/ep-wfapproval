if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalWFinRepCon_HistoryGetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalWFinRepCon_HistoryGetList 
GO
...
-- Returns a query resultset from table [dbo].[WFinRepCon_History]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalWFinRepCon_HistoryGetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[WFinRepCon_History] WFinRepCon_History_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY WFinRepCon_History_.[WFRCHi_ID] asc '
        ...
        SET @l_query_select = 'WITH WFinRepCon_History_ AS ( SELECT  '
        SET @l_query_rownum = 'ROW_NUMBER() OVER(' + @l_sort_str + ') AS IS_ROWNUM_COL,'
        SET @l_query_cols = 
            N'WFinRepCon_History_.[WFRCHi_ID],
            WFinRepCon_History_.[WFRCHi_WFRCH_ID],
            WFinRepCon_History_.[WFRCHi_File],
            WFinRepCon_History_.[WFRCHi_CreatedBy],
            WFinRepCon_History_.[WFRCHi_DateCreated],
            CAST(BINARY_CHECKSUM(WFinRepCon_History_.[WFRCHi_ID],WFinRepCon_History_.[WFRCHi_WFRCH_ID],WFinRepCon_History_.[WFRCHi_File],WFinRepCon_History_.[WFRCHi_CreatedBy],WFinRepCon_History_.[WFRCHi_DateCreated]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = ' FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM WFinRepCon_History_ '
    ...
    BEGIN
        ...
        SET @l_query_cols = 
            ...
            CAST(BINARY_CHECKSUM(WFinRepCon_History_.[WFRCHi_ID],WFinRepCon_History_.[WFRCHi_WFRCH_ID],WFinRepCon_History_.[WFRCHi_File],WFinRepCon_History_.[WFRCHi_CreatedBy],WFinRepCon_History_.[WFRCHi_DateCreated]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = 
            ' FROM [dbo].[WFinRepCon_History] WFinRepCon_History_ ' + 
            'WHERE 1=2;'
