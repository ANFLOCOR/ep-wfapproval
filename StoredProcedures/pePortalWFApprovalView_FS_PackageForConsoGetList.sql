if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalView_FS_PackageForConsoGetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalView_FS_PackageForConsoGetList 
GO
...
-- Returns a query resultset from table [dbo].[view_FS_PackageForConso]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalView_FS_PackageForConsoGetList
...
BEGIN
    ...
    BEGIN
        ...
        SET @l_query_select = 'WITH view_FS_PackageForConso_ AS ( SELECT  '
        SET @l_query_rownum = 'ROW_NUMBER() OVER(' + @l_sort_str + ') AS IS_ROWNUM_COL,'
        SET @l_query_cols = 
            N'view_FS_PackageForConso_.[wass_C_ID],
            view_FS_PackageForConso_.[ID],
            view_FS_PackageForConso_.[Year],
            view_FS_PackageForConso_.[Month],
            view_FS_PackageForConso_.[Doc_TypeID],
            view_FS_PackageForConso_.[Submit],
            view_FS_PackageForConso_.[Status],
            view_FS_PackageForConso_.[CompanyID],
            view_FS_PackageForConso_.[UserId],
            view_FS_PackageForConso_.[Remarks],
            view_FS_PackageForConso_.[ReportCnt],
            view_FS_PackageForConso_.[Description],
            view_FS_PackageForConso_.[DescriptionStat],
            view_FS_PackageForConso_.[FS_Type]'

        SET @l_query_from = 'FROM ' + @l_from_str + ' ' + @l_join_str + + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM view_FS_PackageForConso_ '
    ...
    BEGIN
        ...
        SET @l_query_cols = 
            ...
            view_FS_PackageForConso_.[FS_Type]'
        SET @l_query_from = 
            ' FROM [dbo].[view_FS_PackageForConso] view_FS_PackageForConso_ ' + 
            'WHERE 1=2;'
