if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalWPO_Doc_Type1GetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalWPO_Doc_Type1GetList 
GO
...
-- Returns a query resultset from table [dbo].[WPO_Doc_Type]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalWPO_Doc_Type1GetList
...
BEGIN
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY WPO_Doc_Type_.[WPO_DT_ID] asc '
        ...
        SET @l_query_select = 'WITH WPO_Doc_Type_ AS ( SELECT  '
        SET @l_query_rownum = 'ROW_NUMBER() OVER(' + @l_sort_str + ') AS IS_ROWNUM_COL,'
        SET @l_query_cols = 
            N'WPO_Doc_Type_.[WPO_DT_ID],
            WPO_Doc_Type_.[WPO_DT_C_ID],
            WPO_Doc_Type_.[WPO_DT_Name],
            WPO_Doc_Type_.[WPO_DT_Desc],
            WPO_Doc_Type_.[WPO_DT_Type],
            WPO_Doc_Type_.[WPO_DT_Minimum],
            WPO_Doc_Type_.[WPO_DT_Maximum],
            WPO_Doc_Type_.[WPO_DT_Limit],
            CAST(BINARY_CHECKSUM(WPO_Doc_Type_.[WPO_DT_ID],WPO_Doc_Type_.[WPO_DT_C_ID],WPO_Doc_Type_.[WPO_DT_Name],WPO_Doc_Type_.[WPO_DT_Desc],WPO_Doc_Type_.[WPO_DT_Type],WPO_Doc_Type_.[WPO_DT_Minimum],WPO_Doc_Type_.[WPO_DT_Maximum],WPO_Doc_Type_.[WPO_DT_Limit]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = ' FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM WPO_Doc_Type_ '
    ...
    BEGIN
        ...
        SET @l_query_cols = 
            ...
            CAST(BINARY_CHECKSUM(WPO_Doc_Type_.[WPO_DT_ID],WPO_Doc_Type_.[WPO_DT_C_ID],WPO_Doc_Type_.[WPO_DT_Name],WPO_Doc_Type_.[WPO_DT_Desc],WPO_Doc_Type_.[WPO_DT_Type],WPO_Doc_Type_.[WPO_DT_Minimum],WPO_Doc_Type_.[WPO_DT_Maximum],WPO_Doc_Type_.[WPO_DT_Limit]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = 
            ' FROM [dbo].[WPO_Doc_Type] WPO_Doc_Type_ ' + 
            'WHERE 1=2;'
