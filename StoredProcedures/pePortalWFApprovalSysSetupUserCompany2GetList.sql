if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalSysSetupUserCompany2GetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalSysSetupUserCompany2GetList 
GO
...
-- Returns a query resultset from table [dbo].[SysSetupUserCompany]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalSysSetupUserCompany2GetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[SysSetupUserCompany] SysSetupUserCompany_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY SysSetupUserCompany_.[SSUC_RowID] asc '
        ...
        SET @l_query_select = 'WITH SysSetupUserCompany_ AS ( SELECT  '
        SET @l_query_rownum = 'ROW_NUMBER() OVER(' + @l_sort_str + ') AS IS_ROWNUM_COL,'
        SET @l_query_cols = 
            N'SysSetupUserCompany_.[SSUC_SSU_UserName],
            SysSetupUserCompany_.[SSUC_SSC_CompanyID],
            SysSetupUserCompany_.[SSUC_APP_ID],
            SysSetupUserCompany_.[SSUC_isDefault],
            SysSetupUserCompany_.[SSUC_SSUA_RowID],
            SysSetupUserCompany_.[SSUC_RowID],
            CAST(BINARY_CHECKSUM(SysSetupUserCompany_.[SSUC_SSU_UserName],SysSetupUserCompany_.[SSUC_SSC_CompanyID],SysSetupUserCompany_.[SSUC_APP_ID],SysSetupUserCompany_.[SSUC_isDefault],SysSetupUserCompany_.[SSUC_SSUA_RowID],SysSetupUserCompany_.[SSUC_RowID]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = ' FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM SysSetupUserCompany_ '
    ...
    BEGIN
        ...
        SET @l_query_cols = 
            ...
            CAST(BINARY_CHECKSUM(SysSetupUserCompany_.[SSUC_SSU_UserName],SysSetupUserCompany_.[SSUC_SSC_CompanyID],SysSetupUserCompany_.[SSUC_APP_ID],SysSetupUserCompany_.[SSUC_isDefault],SysSetupUserCompany_.[SSUC_SSUA_RowID],SysSetupUserCompany_.[SSUC_RowID]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = 
            ' FROM [dbo].[SysSetupUserCompany] SysSetupUserCompany_ ' + 
            'WHERE 1=2;'
