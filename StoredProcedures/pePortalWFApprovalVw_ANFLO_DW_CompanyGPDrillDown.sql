if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalVw_ANFLO_DW_CompanyGPDrillDown') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalVw_ANFLO_DW_CompanyGPDrillDown 
GO
...
CREATE PROCEDURE pePortalWFApprovalVw_ANFLO_DW_CompanyGPDrillDown
...
BEGIN
    SET NOCOUNT ON
        ...
        SET @l_temp_col_type_query = N'SELECT @data_type_out = DATA_TYPE, @precision_out = NUMERIC_PRECISION, @scale_out = NUMERIC_SCALE, @max_out = CHARACTER_MAXIMUM_LENGTH FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME=''vw_ANFLO_DW_CompanyGP'' and COLUMN_NAME = ' + '''' + @l_temp_col_name + ''''
