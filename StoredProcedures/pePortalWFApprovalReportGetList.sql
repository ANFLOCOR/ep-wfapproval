if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalReportGetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalReportGetList 
GO
...
-- Returns a query resultset from table [dbo].[Report]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalReportGetList
...
BEGIN
    ...
    BEGIN
        ...
        SET @l_query_select = 'WITH Report_ AS ( SELECT  '
        SET @l_query_rownum = 'ROW_NUMBER() OVER(' + @l_sort_str + ') AS IS_ROWNUM_COL,'
        SET @l_query_cols = 
            N'Report_.[ReportID],
            Report_.[Report],
            Report_.[Report_ShortName],
            CAST(BINARY_CHECKSUM(Report_.[ReportID],Report_.[Report],Report_.[Report_ShortName]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = ' FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM Report_ '
