if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalVw_ANFLO_DW_CompanyNonGP1GetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalVw_ANFLO_DW_CompanyNonGP1GetList 
GO
...
-- Returns a query resultset from table [dbo].[vw_ANFLO_DW_CompanyNonGP]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalVw_ANFLO_DW_CompanyNonGP1GetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[vw_ANFLO_DW_CompanyNonGP] vw_ANFLO_DW_CompanyNonGP_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY vw_ANFLO_DW_CompanyNonGP_.[DWCompanyID] asc '
        ...
        SET @l_query_select = 'WITH vw_ANFLO_DW_CompanyNonGP_ AS ( SELECT  '
        SET @l_query_rownum = 'ROW_NUMBER() OVER(' + @l_sort_str + ') AS IS_ROWNUM_COL,'
        SET @l_query_cols = 
            N'vw_ANFLO_DW_CompanyNonGP_.[Name],
            vw_ANFLO_DW_CompanyNonGP_.[ShortName],
            vw_ANFLO_DW_CompanyNonGP_.[INTERID],
            vw_ANFLO_DW_CompanyNonGP_.[DWCompanyID],
            vw_ANFLO_DW_CompanyNonGP_.[DynamicsCompanyID],
            CAST(BINARY_CHECKSUM(vw_ANFLO_DW_CompanyNonGP_.[Name],vw_ANFLO_DW_CompanyNonGP_.[ShortName],vw_ANFLO_DW_CompanyNonGP_.[INTERID],vw_ANFLO_DW_CompanyNonGP_.[DWCompanyID],vw_ANFLO_DW_CompanyNonGP_.[DynamicsCompanyID]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
