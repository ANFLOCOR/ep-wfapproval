...
CREATE PROCEDURE pePortalWFApprovalVw_FS_WFinRep_Attachment_PerReportTypeGet
        @pk_WFRA_ID int
AS
DECLARE
    @l_count int
BEGIN
    ...
    SELECT 
        ...
        [WFRT_SortOrder],
        [FIN_HFIN_ID],
        CAST(BINARY_CHECKSUM([WFRA_ID],[WFRA_FIN_ID],[WFRA_Desc],[WFRA_Doc],[WFRT_Description],[WFRT_SortOrder],[FIN_HFIN_ID]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345
    FROM [dbo].[vw_FS_WFinRep_Attachment_PerReportType]
    WHERE [WFRA_ID] =@pk_WFRA_ID
    SET NOCOUNT OFF
END
