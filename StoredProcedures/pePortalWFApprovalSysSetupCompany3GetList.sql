if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalSysSetupCompany3GetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalSysSetupCompany3GetList 
GO
...
-- Returns a query resultset from table [dbo].[SysSetupCompany]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalSysSetupCompany3GetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[SysSetupCompany] SysSetupCompany_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY SysSetupCompany_.[SSC_CompanyID] asc '
        ...
        SET @l_query_select = 'WITH SysSetupCompany_ AS ( SELECT  '
        SET @l_query_rownum = 'ROW_NUMBER() OVER(' + @l_sort_str + ') AS IS_ROWNUM_COL,'
        SET @l_query_cols = 
            N'SysSetupCompany_.[SSC_CompanyID],
            SysSetupCompany_.[SSC_CompanyName],
            SysSetupCompany_.[SSC_Description],
            SysSetupCompany_.[SSC_Checked],
            SysSetupCompany_.[SSC_DateCreated],
            SysSetupCompany_.[SSC_DateModified],
            CAST(BINARY_CHECKSUM(SysSetupCompany_.[SSC_CompanyID],SysSetupCompany_.[SSC_CompanyName],SysSetupCompany_.[SSC_Description],SysSetupCompany_.[SSC_Checked],SysSetupCompany_.[SSC_DateCreated],SysSetupCompany_.[SSC_DateModified]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = ' FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM SysSetupCompany_ '
