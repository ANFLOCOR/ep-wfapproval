if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalVw_ANFLO_DW_CompanyNonGPGetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalVw_ANFLO_DW_CompanyNonGPGetList 
GO
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalVw_ANFLO_DW_CompanyNonGPGetList
...
BEGIN
    ...
    BEGIN
        ...
        SET @l_temp_colsWithAlias = 
            N'vw_ANFLO_DW_CompanyNonGP_.[CompanyID]'
        SET @l_temp_from = 
            ' FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + 
            @l_where_str + ' ' + 
            @l_sort_str
        ...
        SET @l_query_cols = 
            N'vw_ANFLO_DW_CompanyNonGP_.[CompanyID],
            vw_ANFLO_DW_CompanyNonGP_.[Name],
        ...
        SET @l_query_where = 
            N'ON vw_ANFLO_DW_CompanyNonGP_.[CompanyID] = IS_ALIAS.[CompanyID] ' 
