if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalWCanvass_Detail_InternalDrillDown') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalWCanvass_Detail_InternalDrillDown 
GO
...
CREATE PROCEDURE pePortalWFApprovalWCanvass_Detail_InternalDrillDown
...
BEGIN
    SET NOCOUNT ON
        ...
        SET @l_temp_col_type_query = N'SELECT @data_type_out = DATA_TYPE, @precision_out = NUMERIC_PRECISION, @scale_out = NUMERIC_SCALE, @max_out = CHARACTER_MAXIMUM_LENGTH FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME=''WCanvass_Detail_Internal'' and COLUMN_NAME = ' + '''' + @l_temp_col_name + ''''
