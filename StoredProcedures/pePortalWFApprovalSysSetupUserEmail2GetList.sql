if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalSysSetupUserEmail2GetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalSysSetupUserEmail2GetList 
GO
...
-- Returns a query resultset from table [dbo].[SysSetupUserEmail]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalSysSetupUserEmail2GetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[SysSetupUserEmail] SysSetupUserEmail_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY SysSetupUserEmail_.[SSUE_RowID] asc '
        ...
        SET @l_query_select = 'WITH SysSetupUserEmail_ AS ( SELECT  '
        SET @l_query_rownum = 'ROW_NUMBER() OVER(' + @l_sort_str + ') AS IS_ROWNUM_COL,'
        SET @l_query_cols = 
            N'SysSetupUserEmail_.[SSUE_UserName],
            SysSetupUserEmail_.[SSUE_Email],
            SysSetupUserEmail_.[SSUE_RowID],
            CAST(BINARY_CHECKSUM(SysSetupUserEmail_.[SSUE_UserName],SysSetupUserEmail_.[SSUE_Email],SysSetupUserEmail_.[SSUE_RowID]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = ' FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM SysSetupUserEmail_ '
    ...
    BEGIN
        ...
        SET @l_query_cols = 
            ...
            CAST(BINARY_CHECKSUM(SysSetupUserEmail_.[SSUE_UserName],SysSetupUserEmail_.[SSUE_Email],SysSetupUserEmail_.[SSUE_RowID]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = 
            ' FROM [dbo].[SysSetupUserEmail] SysSetupUserEmail_ ' + 
            'WHERE 1=2;'
