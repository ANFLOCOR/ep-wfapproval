if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalSel_SysSetupUsers_Email1GetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalSel_SysSetupUsers_Email1GetList 
GO
...
-- Returns a query resultset from table [dbo].[sel_SysSetupUsers_Email]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalSel_SysSetupUsers_Email1GetList
...
BEGIN
    ...
    BEGIN
        ...
        SET @l_query_select = 'WITH sel_SysSetupUsers_Email_ AS ( SELECT  '
        SET @l_query_rownum = 'ROW_NUMBER() OVER(' + @l_sort_str + ') AS IS_ROWNUM_COL,'
        SET @l_query_cols = 
            N'sel_SysSetupUsers_Email_.[SSU_UserName],
            sel_SysSetupUsers_Email_.[SSU_FullName],
            sel_SysSetupUsers_Email_.[SSU_Password],
            sel_SysSetupUsers_Email_.[SSU_EmpID],
            sel_SysSetupUsers_Email_.[SSU_SSC_CompanyID],
            sel_SysSetupUsers_Email_.[SSU_IsActive],
            sel_SysSetupUsers_Email_.[SSU_DateCreated],
            sel_SysSetupUsers_Email_.[SSU_DateModified],
            sel_SysSetupUsers_Email_.[SSU_RowID],
            sel_SysSetupUsers_Email_.[SSU_GPUsername],
            sel_SysSetupUsers_Email_.[SSU_Designation],
            sel_SysSetupUsers_Email_.[SSUE_Email]'

        SET @l_query_from = 'FROM ' + @l_from_str + ' ' + @l_join_str + + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM sel_SysSetupUsers_Email_ '
    ...
    BEGIN
        ...
        SET @l_query_cols = 
            ...
            sel_SysSetupUsers_Email_.[SSUE_Email]'
        SET @l_query_from = 
            ' FROM [dbo].[sel_SysSetupUsers_Email] sel_SysSetupUsers_Email_ ' + 
            'WHERE 1=2;'
