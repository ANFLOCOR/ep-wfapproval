if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalVw_WF_DYNAMICS_Company_ALLGetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalVw_WF_DYNAMICS_Company_ALLGetList 
GO
...
-- Returns a query resultset from table [dbo].[vw_WF_DYNAMICS_Company_ALL]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalVw_WF_DYNAMICS_Company_ALLGetList
...
BEGIN
    ...
    BEGIN
        ...
        SET @l_query_select = 'WITH vw_WF_DYNAMICS_Company_ALL_ AS ( SELECT  '
        SET @l_query_rownum = 'ROW_NUMBER() OVER(' + @l_sort_str + ') AS IS_ROWNUM_COL,'
        SET @l_query_cols = 
            N'vw_WF_DYNAMICS_Company_ALL_.[Company_ID],
            vw_WF_DYNAMICS_Company_ALL_.[Company_Desc],
            vw_WF_DYNAMICS_Company_ALL_.[Company_Short_Name],
            vw_WF_DYNAMICS_Company_ALL_.[INTERID],
            vw_WF_DYNAMICS_Company_ALL_.[FULLADDRESS]'

        SET @l_query_from = 'FROM ' + @l_from_str + ' ' + @l_join_str + + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM vw_WF_DYNAMICS_Company_ALL_ '
