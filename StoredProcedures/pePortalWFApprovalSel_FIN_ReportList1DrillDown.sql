if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalSel_FIN_ReportList1DrillDown') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalSel_FIN_ReportList1DrillDown 
GO
...
CREATE PROCEDURE pePortalWFApprovalSel_FIN_ReportList1DrillDown
...
BEGIN
    SET NOCOUNT ON
        ...
        SET @l_temp_col_type_query = N'SELECT @data_type_out = DATA_TYPE, @precision_out = NUMERIC_PRECISION, @scale_out = NUMERIC_SCALE, @max_out = CHARACTER_MAXIMUM_LENGTH FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME=''sel_FIN_ReportList'' and COLUMN_NAME = ' + '''' + @l_temp_col_name + ''''
    ...
    BEGIN
        ...
        SET @l_query_select = 'SELECT '
        SET @l_query_from = 
            ' FROM [dbo].[sel_FIN_ReportList] sel_FIN_ReportList_ ' + 
            'WHERE 1=2;'
