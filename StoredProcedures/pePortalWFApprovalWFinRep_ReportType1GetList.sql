if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalWFinRep_ReportType1GetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalWFinRep_ReportType1GetList 
GO
...
-- Returns a query resultset from table [dbo].[WFinRep_ReportType]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalWFinRep_ReportType1GetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[WFinRep_ReportType] WFinRep_ReportType_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY WFinRep_ReportType_.[WFRT_Type] asc '
        ...
        SET @l_query_select = 'WITH WFinRep_ReportType_ AS ( SELECT  '
        SET @l_query_rownum = 'ROW_NUMBER() OVER(' + @l_sort_str + ') AS IS_ROWNUM_COL,'
        SET @l_query_cols = 
            N'WFinRep_ReportType_.[WFRT_Type],
            WFinRep_ReportType_.[WFRT_Description],
            WFinRep_ReportType_.[WFRT_SortOrder],
            CAST(BINARY_CHECKSUM(WFinRep_ReportType_.[WFRT_Type],WFinRep_ReportType_.[WFRT_Description],WFinRep_ReportType_.[WFRT_SortOrder]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = ' FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM WFinRep_ReportType_ '
    ...
    BEGIN
        ...
        SET @l_query_cols = 
            ...
            CAST(BINARY_CHECKSUM(WFinRep_ReportType_.[WFRT_Type],WFinRep_ReportType_.[WFRT_Description],WFinRep_ReportType_.[WFRT_SortOrder]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = 
            ' FROM [dbo].[WFinRep_ReportType] WFinRep_ReportType_ ' + 
            'WHERE 1=2;'
