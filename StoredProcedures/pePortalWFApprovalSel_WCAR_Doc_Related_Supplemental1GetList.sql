if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalSel_WCAR_Doc_Related_Supplemental1GetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalSel_WCAR_Doc_Related_Supplemental1GetList 
GO
...
-- Returns a query resultset from table [dbo].[sel_WCAR_Doc_Related_Supplemental]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalSel_WCAR_Doc_Related_Supplemental1GetList
...
BEGIN
    ...
    BEGIN
        ...
        SET @l_query_select = 'WITH sel_WCAR_Doc_Related_Supplemental_ AS ( SELECT  '
        SET @l_query_rownum = 'ROW_NUMBER() OVER(' + @l_sort_str + ') AS IS_ROWNUM_COL,'
        SET @l_query_cols = 
            N'sel_WCAR_Doc_Related_Supplemental_.[WCD_ID],
            sel_WCAR_Doc_Related_Supplemental_.[Src_CAR],
            sel_WCAR_Doc_Related_Supplemental_.[WCD_Supplementary],
            sel_WCAR_Doc_Related_Supplemental_.[WCD_No],
            sel_WCAR_Doc_Related_Supplemental_.[WCD_Project_Title],
            sel_WCAR_Doc_Related_Supplemental_.[WCD_Request_Date],
            sel_WCAR_Doc_Related_Supplemental_.[WCD_Exp_Total],
            sel_WCAR_Doc_Related_Supplemental_.[Rel_CAR],
            sel_WCAR_Doc_Related_Supplemental_.[Rel_Proj],
            sel_WCAR_Doc_Related_Supplemental_.[Rel_Req_Date],
            sel_WCAR_Doc_Related_Supplemental_.[Rel_Total],
            sel_WCAR_Doc_Related_Supplemental_.[WCD_C_ID]'

        SET @l_query_from = 'FROM ' + @l_from_str + ' ' + @l_join_str + + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM sel_WCAR_Doc_Related_Supplemental_ '
    ...
    BEGIN
        ...
        SET @l_query_cols = 
            ...
            sel_WCAR_Doc_Related_Supplemental_.[WCD_C_ID]'
        SET @l_query_from = 
            ' FROM [dbo].[sel_WCAR_Doc_Related_Supplemental] sel_WCAR_Doc_Related_Supplemental_ ' + 
            'WHERE 1=2;'
