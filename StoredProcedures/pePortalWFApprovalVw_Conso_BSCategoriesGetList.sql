if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalVw_Conso_BSCategoriesGetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalVw_Conso_BSCategoriesGetList 
GO
...
-- Returns a query resultset from table [dbo].[vw_Conso_BSCategories]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalVw_Conso_BSCategoriesGetList
...
BEGIN
    ...
    BEGIN
        ...
        SET @l_query_select = 'WITH vw_Conso_BSCategories_ AS ( SELECT  '
        SET @l_query_rownum = 'ROW_NUMBER() OVER(' + @l_sort_str + ') AS IS_ROWNUM_COL,'
        SET @l_query_cols = 
            N'vw_Conso_BSCategories_.[ACCATROW_ID],
            vw_Conso_BSCategories_.[ACCATNUM],
            vw_Conso_BSCategories_.[ACCATDESC],
            vw_Conso_BSCategories_.[ORDERNO],
            vw_Conso_BSCategories_.[LEVELNO],
            vw_Conso_BSCategories_.[LEVEL1],
            vw_Conso_BSCategories_.[LEVEL1_ORD],
            vw_Conso_BSCategories_.[LEVEL2],
            vw_Conso_BSCategories_.[LEVEL2_ORD]'

        SET @l_query_from = 'FROM ' + @l_from_str + ' ' + @l_join_str + + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM vw_Conso_BSCategories_ '
    ...
    BEGIN
        ...
        SET @l_query_cols = 
            ...
            vw_Conso_BSCategories_.[LEVEL2_ORD]'
        SET @l_query_from = 
            ' FROM [dbo].[vw_Conso_BSCategories] vw_Conso_BSCategories_ ' + 
            'WHERE 1=2;'
