if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalWPO_CARNo_QWF1GetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalWPO_CARNo_QWF1GetList 
GO
...
-- Returns a query resultset from table [dbo].[WPO_CARNo_QWF]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalWPO_CARNo_QWF1GetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[WPO_CARNo_QWF] WPO_CARNo_QWF_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY WPO_CARNo_QWF_.[PONum],WPO_CARNo_QWF_.[PRNum] asc '
        ...
        SET @l_temp_colsWithAlias = 
            N'WPO_CARNo_QWF_.[PONum],WPO_CARNo_QWF_.[PRNum]'
        SET @l_temp_from = 
            ' FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + 
            @l_where_str + ' ' + 
            @l_sort_str
        ...
        SET @l_query_cols = 
            N'WPO_CARNo_QWF_.[CompanyID],
            WPO_CARNo_QWF_.[PONum],
            WPO_CARNo_QWF_.[PRNum],
            WPO_CARNo_QWF_.[Comment],
            WPO_CARNo_QWF_.[CARID],
            WPO_CARNo_QWF_.[WCD_No],
            WPO_CARNo_QWF_.[WCD_Remark],
            WPO_CARNo_QWF_.[WCD_Project_Title],
            WPO_CARNo_QWF_.[WCD_Exp_Total],
            WPO_CARNo_QWF_.[WCD_ID],
            CAST(BINARY_CHECKSUM(WPO_CARNo_QWF_.[CompanyID],WPO_CARNo_QWF_.[PONum],WPO_CARNo_QWF_.[PRNum],WPO_CARNo_QWF_.[Comment],WPO_CARNo_QWF_.[CARID],WPO_CARNo_QWF_.[WCD_No],WPO_CARNo_QWF_.[WCD_Remark],WPO_CARNo_QWF_.[WCD_Project_Title],WPO_CARNo_QWF_.[WCD_Exp_Total],WPO_CARNo_QWF_.[WCD_ID]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = 
            ' FROM ( ' +
        ...
        SET @l_query_where = 
            N'ON WPO_CARNo_QWF_.[PONum] = IS_ALIAS.[PONum] AND WPO_CARNo_QWF_.[PRNum] = IS_ALIAS.[PRNum] ' 
    ...
    BEGIN
        ...
        SET @l_query_cols = 
            ...
            CAST(BINARY_CHECKSUM(WPO_CARNo_QWF_.[CompanyID],WPO_CARNo_QWF_.[PONum],WPO_CARNo_QWF_.[PRNum],WPO_CARNo_QWF_.[Comment],WPO_CARNo_QWF_.[CARID],WPO_CARNo_QWF_.[WCD_No],WPO_CARNo_QWF_.[WCD_Remark],WPO_CARNo_QWF_.[WCD_Project_Title],WPO_CARNo_QWF_.[WCD_Exp_Total],WPO_CARNo_QWF_.[WCD_ID]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = 
            ' FROM [dbo].[WPO_CARNo_QWF] WPO_CARNo_QWF_ ' + 
            'WHERE 1=2;'
