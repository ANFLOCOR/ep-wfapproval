if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalSel_SysSetupUsers_UserRoles1GetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalSel_SysSetupUsers_UserRoles1GetList 
GO
...
-- Returns a query resultset from table [dbo].[sel_SysSetupUsers_UserRoles]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalSel_SysSetupUsers_UserRoles1GetList
...
BEGIN
    ...
    BEGIN
        ...
        SET @l_query_select = 'WITH sel_SysSetupUsers_UserRoles_ AS ( SELECT  '
        SET @l_query_rownum = 'ROW_NUMBER() OVER(' + @l_sort_str + ') AS IS_ROWNUM_COL,'
        SET @l_query_cols = 
            N'sel_SysSetupUsers_UserRoles_.[SSU_UserName],
            sel_SysSetupUsers_UserRoles_.[SSU_RowID],
            sel_SysSetupUsers_UserRoles_.[SSUA_App_ID],
            sel_SysSetupUsers_UserRoles_.[SSUC_SSC_CompanyID],
            sel_SysSetupUsers_UserRoles_.[SSUP_SSR_RoleID],
            sel_SysSetupUsers_UserRoles_.[SSUP_RowID],
            sel_SysSetupUsers_UserRoles_.[SSR_RoleName],
            sel_SysSetupUsers_UserRoles_.[SSUC_RowID],
            sel_SysSetupUsers_UserRoles_.[SSUA_RowID],
            sel_SysSetupUsers_UserRoles_.[SSUP_Permission]'

        SET @l_query_from = 'FROM ' + @l_from_str + ' ' + @l_join_str + + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM sel_SysSetupUsers_UserRoles_ '
