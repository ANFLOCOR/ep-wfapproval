if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalSel_IV407001GetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalSel_IV407001GetList 
GO
...
-- Returns a query resultset from table [dbo].[sel_IV40700]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalSel_IV407001GetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[sel_IV40700] sel_IV40700_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY sel_IV40700_.[LOCNCODE],sel_IV40700_.[Company_ID] asc '
        ...
        IS_ROWNUM_COL int identity(1,1),
                [LOCNCODE] char(11) COLLATE Latin1_General_BIN,    [Company_ID] int
        ); '
        ...
        SET @l_temp_cols = 
            N'[LOCNCODE],[Company_ID]'
        SET @l_temp_select = 
            ') ' + 
            'SELECT ' + 
            'TOP ' + convert(varchar, @l_end_gen_row_num) + ' '
        SET @l_temp_colsWithAlias = 
            N'sel_IV40700_.[LOCNCODE],sel_IV40700_.[Company_ID]'
        SET @l_temp_from = 
            ' FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + 
            @l_where_str + ' ' + 
            @l_sort_str
        ...
        SET @l_query_cols = 
            N'sel_IV40700_.[LOCNCODE],
            sel_IV40700_.[LOCNDSCR],
            sel_IV40700_.[Company_ID],
            CAST(BINARY_CHECKSUM(sel_IV40700_.[LOCNCODE],sel_IV40700_.[LOCNDSCR],sel_IV40700_.[Company_ID]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = 
            ' FROM ( ' +
                N'SELECT TOP 100 PERCENT IS_ROWNUM_COL, [LOCNCODE],[Company_ID] from @IS_TEMP_T_GETLIST ' +
                'WHERE IS_ROWNUM_COL >= '+ convert(varchar, @l_start_gen_row_num) + 
                ') IS_ALIAS LEFT JOIN ' +
                @l_from_str + ' ';

        SET @l_query_where = 
            N'ON sel_IV40700_.[LOCNCODE] = IS_ALIAS.[LOCNCODE] AND sel_IV40700_.[Company_ID] = IS_ALIAS.[Company_ID] ' 
    ...
    BEGIN
        ...
        SET @l_query_cols = 
            ...
            CAST(BINARY_CHECKSUM(sel_IV40700_.[LOCNCODE],sel_IV40700_.[LOCNDSCR],sel_IV40700_.[Company_ID]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = 
            ' FROM [dbo].[sel_IV40700] sel_IV40700_ ' + 
            'WHERE 1=2;'
