if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalVw_WFinRep_DocAttach_FIN_Month1GetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalVw_WFinRep_DocAttach_FIN_Month1GetList 
GO
...
-- Returns a query resultset from table [dbo].[vw_WFinRep_DocAttach_FIN_Month]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalVw_WFinRep_DocAttach_FIN_Month1GetList
...
BEGIN
    ...
    BEGIN
        ...
        SET @l_query_select = 'WITH Vw_WFinRep_DocAttac_671747064_ AS ( SELECT  '
        SET @l_query_rownum = 'ROW_NUMBER() OVER(' + @l_sort_str + ') AS IS_ROWNUM_COL,'
        SET @l_query_cols = 
            N'Vw_WFinRep_DocAttac_671747064_.[Mo],
            Vw_WFinRep_DocAttac_671747064_.[MoName],
            CAST(BINARY_CHECKSUM(Vw_WFinRep_DocAttac_671747064_.[Mo],Vw_WFinRep_DocAttac_671747064_.[MoName]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = ' FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM Vw_WFinRep_DocAttac_671747064_ '
