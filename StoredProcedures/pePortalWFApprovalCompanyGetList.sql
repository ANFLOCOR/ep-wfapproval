if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalCompanyGetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalCompanyGetList 
GO
...
-- Returns a query resultset from table [dbo].[Company]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalCompanyGetList
...
BEGIN
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY Company_.[CompanyID] asc '
        ...
        SET @l_query_select = 'WITH Company_ AS ( SELECT  '
        SET @l_query_rownum = 'ROW_NUMBER() OVER(' + @l_sort_str + ') AS IS_ROWNUM_COL,'
        SET @l_query_cols = 
            N'Company_.[CompanyID],
            Company_.[Name],
            Company_.[ShortName],
            Company_.[GroupID],
            Company_.[IsNonGP],
            Company_.[DynamicsCompanyID],
            Company_.[CreatedBy],
            Company_.[DateCreated],
            Company_.[ModifiedBy],
            Company_.[DateModified],
            Company_.[GPInterID],
            Company_.[IsConso],
            CAST(BINARY_CHECKSUM(Company_.[CompanyID],Company_.[Name],Company_.[ShortName],Company_.[GroupID],Company_.[IsNonGP],Company_.[DynamicsCompanyID],Company_.[CreatedBy],Company_.[DateCreated],Company_.[ModifiedBy],Company_.[DateModified],Company_.[GPInterID],Company_.[IsConso]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = ' FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM Company_ '
