if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalSel_Dynamics_CompanyAllGetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalSel_Dynamics_CompanyAllGetList 
GO
...
-- Returns a query resultset from table [dbo].[sel_Dynamics_CompanyAll]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalSel_Dynamics_CompanyAllGetList
...
BEGIN
    ...
    BEGIN
        ...
        SET @l_query_select = 'WITH sel_Dynamics_CompanyAll_ AS ( SELECT  '
        SET @l_query_rownum = 'ROW_NUMBER() OVER(' + @l_sort_str + ') AS IS_ROWNUM_COL,'
        SET @l_query_cols = 
            N'sel_Dynamics_CompanyAll_.[CompanyID],
            sel_Dynamics_CompanyAll_.[Company_Desc],
            sel_Dynamics_CompanyAll_.[Company_Short_Name],
            sel_Dynamics_CompanyAll_.[INTERID],
            sel_Dynamics_CompanyAll_.[FULLADDRESS]'

        SET @l_query_from = 'FROM ' + @l_from_str + ' ' + @l_join_str + + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM sel_Dynamics_CompanyAll_ '
