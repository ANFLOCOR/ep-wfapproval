if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalSel_WPO_POP101001GetStats') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalSel_WPO_POP101001GetStats 
GO
...
-- sum, avg, max, etc
CREATE PROCEDURE pePortalWFApprovalSel_WPO_POP101001GetStats
    ...
    BEGIN
        ...
        SET @l_temp_col_type_query = N'SELECT @data_type_out = DATA_TYPE, @precision_out = NUMERIC_PRECISION, @scale_out = NUMERIC_SCALE FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME=''sel_WPO_POP10100'' and COLUMN_NAME = ' + '''' + @l_temp_col_name + ''''
        ...
        BEGIN
            SET @l_query = 
                'SELECT count(*) from ' + '[dbo].[sel_WPO_POP10100] sel_WPO_POP10100_ ' +
                'WHERE 1=2;'
            EXECUTE (@l_query)
        END
        SET NOCOUNT OFF
