if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalSel_WPO_WFTaskActivity_RemarksGetStats') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalSel_WPO_WFTaskActivity_RemarksGetStats 
GO
...
-- sum, avg, max, etc
CREATE PROCEDURE pePortalWFApprovalSel_WPO_WFTaskActivity_RemarksGetStats
    ...
    BEGIN
        ...
        SET @l_temp_col_type_query = N'SELECT @data_type_out = DATA_TYPE, @precision_out = NUMERIC_PRECISION, @scale_out = NUMERIC_SCALE FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME=''sel_WPO_WFTaskActivity_Remarks'' and COLUMN_NAME = ' + '''' + @l_temp_col_name + ''''
        ...
        BEGIN
            ...
            SET @l_query = 
                ...
                ') ' + 'Sel_WPO_WFTaskActivi792457005_'
        ...
        BEGIN
            SET @l_query = 
                'SELECT count(*) from ' + '[dbo].[sel_WPO_WFTaskActivity_Remarks] Sel_WPO_WFTaskActivi792457005_ ' +
                'WHERE 1=2;'
            EXECUTE (@l_query)
        END
        SET NOCOUNT OFF
