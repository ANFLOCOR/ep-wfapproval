if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalVw_Trx_Summary_GroupByCompanyYearPeriodDrillDown') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalVw_Trx_Summary_GroupByCompanyYearPeriodDrillDown 
GO
...
CREATE PROCEDURE pePortalWFApprovalVw_Trx_Summary_GroupByCompanyYearPeriodDrillDown
        @p_select_str nvarchar(max),
        @p_is_distinct int,
        @p_select_str_b nvarchar(max),
...
DECLARE
    ...
    @l_temp_col nvarchar(max),
    @l_insert_to_temp nvarchar(max),
...
BEGIN
    SET NOCOUNT ON
        ...
        SET @l_temp_col_type_query = N'SELECT @data_type_out = DATA_TYPE, @precision_out = NUMERIC_PRECISION, @scale_out = NUMERIC_SCALE, @max_out = CHARACTER_MAXIMUM_LENGTH FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME=''vw_Trx_Summary_GroupByCompanyYearPeriod'' and COLUMN_NAME = ' + '''' + @l_temp_col_name + ''''
    ...
    BEGIN
        ...
        SET @l_query_from = 
            ...
            'AS [vw_Trx_Summary_GroupByCompanyYearPeriod_]' +
