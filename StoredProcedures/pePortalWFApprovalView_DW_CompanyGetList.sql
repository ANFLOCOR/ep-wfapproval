if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalView_DW_CompanyGetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalView_DW_CompanyGetList 
GO
...
-- Returns a query resultset from table [dbo].[View_DW_Company]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalView_DW_CompanyGetList
...
BEGIN
    ...
    BEGIN
        ...
        SET @l_query_select = 'WITH View_DW_Company_ AS ( SELECT  '
        SET @l_query_rownum = 'ROW_NUMBER() OVER(' + @l_sort_str + ') AS IS_ROWNUM_COL,'
        SET @l_query_cols = 
            N'View_DW_Company_.[wass_C_ID],
            View_DW_Company_.[DW_C_ID],
            View_DW_Company_.[Name],
            View_DW_Company_.[ShortName],
            View_DW_Company_.[GroupID],
            View_DW_Company_.[IsNonGP],
            View_DW_Company_.[DynamicsCompanyID],
            View_DW_Company_.[GPInterID]'

        SET @l_query_from = 'FROM ' + @l_from_str + ' ' + @l_join_str + + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM View_DW_Company_ '
    ...
    BEGIN
        ...
        SET @l_query_cols = 
            ...
            View_DW_Company_.[GPInterID]'
        SET @l_query_from = 
            ' FROM [dbo].[View_DW_Company] View_DW_Company_ ' + 
            'WHERE 1=2;'
