if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalSysSetupUserCompany_view1GetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalSysSetupUserCompany_view1GetList 
GO
...
-- Returns a query resultset from table [dbo].[SysSetupUserCompany_view]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalSysSetupUserCompany_view1GetList
...
BEGIN
    ...
    BEGIN
        ...
        SET @l_query_select = 'WITH SysSetupUserCompany_view_ AS ( SELECT  '
        SET @l_query_rownum = 'ROW_NUMBER() OVER(' + @l_sort_str + ') AS IS_ROWNUM_COL,'
        SET @l_query_cols = 
            N'SysSetupUserCompany_view_.[SSUC_SSU_UserName],
            SysSetupUserCompany_view_.[SSUC_SSC_CompanyID],
            SysSetupUserCompany_view_.[SSUC_APP_ID],
            SysSetupUserCompany_view_.[SSUC_isDefault],
            SysSetupUserCompany_view_.[SSUC_SSUA_RowID],
            SysSetupUserCompany_view_.[SSUC_RowID],
            SysSetupUserCompany_view_.[SSC_CompanyName],
            SysSetupUserCompany_view_.[SSC_Description],
            SysSetupUserCompany_view_.[INTERID]'

        SET @l_query_from = 'FROM ' + @l_from_str + ' ' + @l_join_str + + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM SysSetupUserCompany_view_ '
    ...
    BEGIN
        ...
        SET @l_query_cols = 
            ...
            SysSetupUserCompany_view_.[INTERID]'
        SET @l_query_from = 
            ' FROM [dbo].[SysSetupUserCompany_view] SysSetupUserCompany_view_ ' + 
            'WHERE 1=2;'
