if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalW_Module_SourceGetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalW_Module_SourceGetList 
GO
...
-- Returns a query resultset from table [dbo].[W_Module_Source]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalW_Module_SourceGetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[W_Module_Source] W_Module_Source_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY W_Module_Source_.[W_MS_ID] asc '
        ...
        SET @l_query_select = 'WITH W_Module_Source_ AS ( SELECT  '
        SET @l_query_rownum = 'ROW_NUMBER() OVER(' + @l_sort_str + ') AS IS_ROWNUM_COL,'
        SET @l_query_cols = 
            N'W_Module_Source_.[W_MS_ID],
            W_Module_Source_.[W_MS_A_ID],
            W_Module_Source_.[W_MS_Name],
            W_Module_Source_.[W_MS_Desc],
            CAST(BINARY_CHECKSUM(W_Module_Source_.[W_MS_ID],W_Module_Source_.[W_MS_A_ID],W_Module_Source_.[W_MS_Name],W_Module_Source_.[W_MS_Desc]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = ' FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM W_Module_Source_ '
    ...
    BEGIN
        ...
        SET @l_query_cols = 
            ...
            CAST(BINARY_CHECKSUM(W_Module_Source_.[W_MS_ID],W_Module_Source_.[W_MS_A_ID],W_Module_Source_.[W_MS_Name],W_Module_Source_.[W_MS_Desc]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = 
            ' FROM [dbo].[W_Module_Source] W_Module_Source_ ' + 
            'WHERE 1=2;'
