if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalWFin_ApprovalStatus1GetStats') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalWFin_ApprovalStatus1GetStats 
GO
...
-- sum, avg, max, etc
CREATE PROCEDURE pePortalWFApprovalWFin_ApprovalStatus1GetStats
    ...
    BEGIN
        ...
        SET @l_temp_col_type_query = N'SELECT @data_type_out = DATA_TYPE, @precision_out = NUMERIC_PRECISION, @scale_out = NUMERIC_SCALE FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME=''WFin_ApprovalStatus'' and COLUMN_NAME = ' + '''' + @l_temp_col_name + ''''
        ...
        SET @l_from_str = '[dbo].[WFin_ApprovalStatus] WFin_ApprovalStatus_'
        ...
        BEGIN
            ...
            ELSE
                SET @l_sort_str = N'ORDER BY WFin_ApprovalStatus_.[WPO_STAT_CD] asc '
        ...
        BEGIN
            SET @l_query = 
                'SELECT count(*) from ' + '[dbo].[WFin_ApprovalStatus] WFin_ApprovalStatus_ ' +
                'WHERE 1=2;'
            EXECUTE (@l_query)
        END
        SET NOCOUNT OFF
