if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalView_FS_StatusAllCompanyGetStats') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalView_FS_StatusAllCompanyGetStats 
GO
...
-- sum, avg, max, etc
CREATE PROCEDURE pePortalWFApprovalView_FS_StatusAllCompanyGetStats
    ...
    BEGIN
        ...
        SET @l_temp_col_type_query = N'SELECT @data_type_out = DATA_TYPE, @precision_out = NUMERIC_PRECISION, @scale_out = NUMERIC_SCALE FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME=''View_FS_StatusAllCompany'' and COLUMN_NAME = ' + '''' + @l_temp_col_name + ''''
        ...
        BEGIN
            SET @l_query = 
                'SELECT count(*) from ' + '[dbo].[View_FS_StatusAllCompany] View_FS_StatusAllCompany_ ' +
                'WHERE 1=2;'
            EXECUTE (@l_query)
        END
        SET NOCOUNT OFF
