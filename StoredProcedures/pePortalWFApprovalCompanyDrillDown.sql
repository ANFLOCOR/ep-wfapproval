if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalCompanyDrillDown') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalCompanyDrillDown 
GO
...
CREATE PROCEDURE pePortalWFApprovalCompanyDrillDown
...
BEGIN
    SET NOCOUNT ON
        ...
        SET @l_temp_col_type_query = N'SELECT @data_type_out = DATA_TYPE, @precision_out = NUMERIC_PRECISION, @scale_out = NUMERIC_SCALE, @max_out = CHARACTER_MAXIMUM_LENGTH FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME=''Company'' and COLUMN_NAME = ' + '''' + @l_temp_col_name + ''''
