if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalWFinRep_DocType1GetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalWFinRep_DocType1GetList 
GO
...
-- Returns a query resultset from table [dbo].[WFinRep_DocType]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalWFinRep_DocType1GetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[WFinRep_DocType] WFinRep_DocType_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY WFinRep_DocType_.[WFIN_DT_ID] asc '
        ...
        SET @l_query_select = 'WITH WFinRep_DocType_ AS ( SELECT  '
        SET @l_query_rownum = 'ROW_NUMBER() OVER(' + @l_sort_str + ') AS IS_ROWNUM_COL,'
        SET @l_query_cols = 
            N'WFinRep_DocType_.[WFIN_DT_ID],
            WFinRep_DocType_.[WFIN_DT_C_ID],
            WFinRep_DocType_.[WFIN_DT_Name],
            WFinRep_DocType_.[WFIN_DT_Desc],
            WFinRep_DocType_.[WFIN_DT_Type],
            WFinRep_DocType_.[WFIN_DT_Minimum],
            WFinRep_DocType_.[WFIN_DT_Maximum],
            CAST(BINARY_CHECKSUM(WFinRep_DocType_.[WFIN_DT_ID],WFinRep_DocType_.[WFIN_DT_C_ID],WFinRep_DocType_.[WFIN_DT_Name],WFinRep_DocType_.[WFIN_DT_Desc],WFinRep_DocType_.[WFIN_DT_Type],WFinRep_DocType_.[WFIN_DT_Minimum],WFinRep_DocType_.[WFIN_DT_Maximum]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = ' FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM WFinRep_DocType_ '
    ...
    BEGIN
        ...
        SET @l_query_cols = 
            ...
            CAST(BINARY_CHECKSUM(WFinRep_DocType_.[WFIN_DT_ID],WFinRep_DocType_.[WFIN_DT_C_ID],WFinRep_DocType_.[WFIN_DT_Name],WFinRep_DocType_.[WFIN_DT_Desc],WFinRep_DocType_.[WFIN_DT_Type],WFinRep_DocType_.[WFIN_DT_Minimum],WFinRep_DocType_.[WFIN_DT_Maximum]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = 
            ' FROM [dbo].[WFinRep_DocType] WFinRep_DocType_ ' + 
            'WHERE 1=2;'
