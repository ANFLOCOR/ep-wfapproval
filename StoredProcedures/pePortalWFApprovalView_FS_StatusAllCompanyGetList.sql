if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalView_FS_StatusAllCompanyGetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalView_FS_StatusAllCompanyGetList 
GO
...
-- Returns a query resultset from table [dbo].[View_FS_StatusAllCompany]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalView_FS_StatusAllCompanyGetList
...
BEGIN
    ...
    BEGIN
        ...
        SET @l_query_select = 'WITH View_FS_StatusAllCompany_ AS ( SELECT  '
        SET @l_query_rownum = 'ROW_NUMBER() OVER(' + @l_sort_str + ') AS IS_ROWNUM_COL,'
        SET @l_query_cols = 
            N'View_FS_StatusAllCompany_.[wass_C_ID],
            View_FS_StatusAllCompany_.[ID],
            View_FS_StatusAllCompany_.[Year],
            View_FS_StatusAllCompany_.[Month],
            View_FS_StatusAllCompany_.[Doc_TypeID],
            View_FS_StatusAllCompany_.[Submit],
            View_FS_StatusAllCompany_.[Status],
            View_FS_StatusAllCompany_.[CompanyID],
            View_FS_StatusAllCompany_.[UserId],
            View_FS_StatusAllCompany_.[Remarks],
            View_FS_StatusAllCompany_.[ReportCnt],
            View_FS_StatusAllCompany_.[Description],
            View_FS_StatusAllCompany_.[DescriptionStat],
            View_FS_StatusAllCompany_.[FS_Type]'

        SET @l_query_from = 'FROM ' + @l_from_str + ' ' + @l_join_str + + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM View_FS_StatusAllCompany_ '
    ...
    BEGIN
        ...
        SET @l_query_cols = 
            ...
            View_FS_StatusAllCompany_.[FS_Type]'
        SET @l_query_from = 
            ' FROM [dbo].[View_FS_StatusAllCompany] View_FS_StatusAllCompany_ ' + 
            'WHERE 1=2;'
