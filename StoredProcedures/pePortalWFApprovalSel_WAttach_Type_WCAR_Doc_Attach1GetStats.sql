if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalSel_WAttach_Type_WCAR_Doc_Attach1GetStats') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalSel_WAttach_Type_WCAR_Doc_Attach1GetStats 
GO
...
-- sum, avg, max, etc
CREATE PROCEDURE pePortalWFApprovalSel_WAttach_Type_WCAR_Doc_Attach1GetStats
    ...
    BEGIN
        ...
        SET @l_temp_col_type_query = N'SELECT @data_type_out = DATA_TYPE, @precision_out = NUMERIC_PRECISION, @scale_out = NUMERIC_SCALE FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME=''sel_WAttach_Type_WCAR_Doc_Attach'' and COLUMN_NAME = ' + '''' + @l_temp_col_name + ''''
        ...
        SET @l_from_str = '[dbo].[sel_WAttach_Type_WCAR_Doc_Attach] sel_WAttach_Type_WCAR_Doc_Attach_'
        ...
        BEGIN
            ...
            SET @l_query = 
                ...
                ') ' + 'sel_WAttach_Type_WCAR_Doc_Attach_'
        ...
        BEGIN
            SET @l_query = 
                'SELECT count(*) from ' + '[dbo].[sel_WAttach_Type_WCAR_Doc_Attach] sel_WAttach_Type_WCAR_Doc_Attach_ ' +
                'WHERE 1=2;'
            EXECUTE (@l_query)
        END
        SET NOCOUNT OFF
