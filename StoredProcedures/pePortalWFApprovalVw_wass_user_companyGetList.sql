if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalVw_wass_user_companyGetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalVw_wass_user_companyGetList 
GO
...
-- Returns a query resultset from table [dbo].[vw_wass_user_company]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalVw_wass_user_companyGetList
...
BEGIN
    ...
    BEGIN
        ...
        SET @l_query_select = 'WITH vw_wass_user_company_ AS ( SELECT  '
        SET @l_query_rownum = 'ROW_NUMBER() OVER(' + @l_sort_str + ') AS IS_ROWNUM_COL,'
        SET @l_query_cols = 
            N'vw_wass_user_company_.[SSUC_SSU_UserName],
            vw_wass_user_company_.[SSUC_SSC_CompanyID],
            vw_wass_user_company_.[SSUC_APP_ID],
            vw_wass_user_company_.[SSUC_isDefault],
            vw_wass_user_company_.[SSUC_SSUA_RowID],
            vw_wass_user_company_.[SSUC_RowID],
            vw_wass_user_company_.[CompanyID],
            vw_wass_user_company_.[Name],
            vw_wass_user_company_.[ShortName],
            vw_wass_user_company_.[INTERID],
            vw_wass_user_company_.[FULLADDRESS],
            vw_wass_user_company_.[DWCompanyID]'

        SET @l_query_from = 'FROM ' + @l_from_str + ' ' + @l_join_str + + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM vw_wass_user_company_ '
    ...
    BEGIN
        ...
        SET @l_query_cols = 
            ...
            vw_wass_user_company_.[DWCompanyID]'
        SET @l_query_from = 
            ' FROM [dbo].[vw_wass_user_company] vw_wass_user_company_ ' + 
            'WHERE 1=2;'
