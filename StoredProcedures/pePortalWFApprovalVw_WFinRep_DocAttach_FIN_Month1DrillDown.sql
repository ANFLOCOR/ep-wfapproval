if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalVw_WFinRep_DocAttach_FIN_Month1DrillDown') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalVw_WFinRep_DocAttach_FIN_Month1DrillDown 
GO
...
CREATE PROCEDURE pePortalWFApprovalVw_WFinRep_DocAttach_FIN_Month1DrillDown
...
BEGIN
    SET NOCOUNT ON
        ...
        SET @l_temp_col_type_query = N'SELECT @data_type_out = DATA_TYPE, @precision_out = NUMERIC_PRECISION, @scale_out = NUMERIC_SCALE, @max_out = CHARACTER_MAXIMUM_LENGTH FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME=''vw_WFinRep_DocAttach_FIN_Month'' and COLUMN_NAME = ' + '''' + @l_temp_col_name + ''''
    ...
    BEGIN
        ...
        SET @l_query_from = 
            ...
            'AS [Vw_WFinRep_DocAttac_671747064_]' +
