if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalSysPages3GetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalSysPages3GetList 
GO
...
-- Returns a query resultset from table [dbo].[SysPages]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalSysPages3GetList
...
BEGIN
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY SysPages_.[PageID] asc '
        ...
        SET @l_query_select = 'WITH SysPages_ AS ( SELECT  '
        ...
        SET @l_query_cols = 
            ...
            SysPages_.[Level],
            SysPages_.[URL],
            SysPages_.[DateCreated],
            SysPages_.[Sorting],
            SysPages_.[isDisplay],
            SysPages_.[Checked],
            SysPages_.[SystemID],
            CAST(BINARY_CHECKSUM(SysPages_.[PageID],SysPages_.[PageName],SysPages_.[ParentID],SysPages_.[Level],SysPages_.[URL],SysPages_.[DateCreated],SysPages_.[Sorting],SysPages_.[isDisplay],SysPages_.[Checked],SysPages_.[SystemID]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = ' FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM SysPages_ '
