if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalWPO_Doc_Type1Update') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalWPO_Doc_Type1Update 
GO
...
CREATE PROCEDURE pePortalWFApprovalWPO_Doc_Type1Update
    @pk_WPO_DT_ID int,
...
-- Check whether the record still exists before doing update
    ...
    ELSE
        BEGIN
            ...
            Select @l_newValue = CAST(BINARY_CHECKSUM([WPO_DT_ID],[WPO_DT_C_ID],[WPO_DT_Name],[WPO_DT_Desc],[WPO_DT_Type],[WPO_DT_Minimum],[WPO_DT_Maximum],[WPO_DT_Limit]) AS nvarchar(max)) 
