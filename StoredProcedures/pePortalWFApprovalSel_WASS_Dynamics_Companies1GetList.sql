if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalSel_WASS_Dynamics_Companies1GetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalSel_WASS_Dynamics_Companies1GetList 
GO
...
-- Returns a query resultset from table [dbo].[sel_WASS_Dynamics_Companies]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalSel_WASS_Dynamics_Companies1GetList
...
BEGIN
    ...
    BEGIN
        ...
        SET @l_query_select = 'WITH sel_WASS_Dynamics_Companies_ AS ( SELECT  '
        SET @l_query_rownum = 'ROW_NUMBER() OVER(' + @l_sort_str + ') AS IS_ROWNUM_COL,'
        SET @l_query_cols = 
            N'sel_WASS_Dynamics_Companies_.[SSC_CompanyName],
            sel_WASS_Dynamics_Companies_.[SSC_Description],
            sel_WASS_Dynamics_Companies_.[INTERID],
            sel_WASS_Dynamics_Companies_.[SSC_CompanyID]'

        SET @l_query_from = 'FROM ' + @l_from_str + ' ' + @l_join_str + + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM sel_WASS_Dynamics_Companies_ '
