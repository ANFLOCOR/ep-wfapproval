if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalSysSetupCompany3GetStats') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalSysSetupCompany3GetStats 
GO
...
-- sum, avg, max, etc
CREATE PROCEDURE pePortalWFApprovalSysSetupCompany3GetStats
    ...
    BEGIN
        ...
        SET @l_temp_col_type_query = N'SELECT @data_type_out = DATA_TYPE, @precision_out = NUMERIC_PRECISION, @scale_out = NUMERIC_SCALE FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME=''SysSetupCompany'' and COLUMN_NAME = ' + '''' + @l_temp_col_name + ''''
        ...
        BEGIN
            SET @l_query = 
                'SELECT count(*) from ' + '[dbo].[SysSetupCompany] SysSetupCompany_ ' +
                'WHERE 1=2;'
            EXECUTE (@l_query)
        END
        SET NOCOUNT OFF
