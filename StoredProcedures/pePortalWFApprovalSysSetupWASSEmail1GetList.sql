if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalSysSetupWASSEmail1GetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalSysSetupWASSEmail1GetList 
GO
...
-- Returns a query resultset from table [dbo].[SysSetupWASSEmail]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalSysSetupWASSEmail1GetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[SysSetupWASSEmail] SysSetupWASSEmail_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY SysSetupWASSEmail_.[WE_ID] asc '
        ...
        SET @l_query_select = 'WITH SysSetupWASSEmail_ AS ( SELECT  '
        SET @l_query_rownum = 'ROW_NUMBER() OVER(' + @l_sort_str + ') AS IS_ROWNUM_COL,'
        SET @l_query_cols = 
            N'SysSetupWASSEmail_.[WE_ID],
            SysSetupWASSEmail_.[WE_U_ID],
            SysSetupWASSEmail_.[WE_Directory],
            SysSetupWASSEmail_.[WE_Site],
            SysSetupWASSEmail_.[WE_Template],
            CAST(BINARY_CHECKSUM(SysSetupWASSEmail_.[WE_ID],SysSetupWASSEmail_.[WE_U_ID],SysSetupWASSEmail_.[WE_Directory],SysSetupWASSEmail_.[WE_Site],SysSetupWASSEmail_.[WE_Template]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = ' FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM SysSetupWASSEmail_ '
    ...
    BEGIN
        ...
        SET @l_query_cols = 
            ...
            CAST(BINARY_CHECKSUM(SysSetupWASSEmail_.[WE_ID],SysSetupWASSEmail_.[WE_U_ID],SysSetupWASSEmail_.[WE_Directory],SysSetupWASSEmail_.[WE_Site],SysSetupWASSEmail_.[WE_Template]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = 
            ' FROM [dbo].[SysSetupWASSEmail] SysSetupWASSEmail_ ' + 
            'WHERE 1=2;'
