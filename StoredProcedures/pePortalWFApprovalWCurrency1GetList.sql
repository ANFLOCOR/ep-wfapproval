if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalWCurrency1GetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalWCurrency1GetList 
GO
...
-- Returns a query resultset from table [dbo].[WCurrency]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalWCurrency1GetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[WCurrency] WCurrency_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY WCurrency_.[WCur_ID] asc '
        ...
        SET @l_query_cols = 
            N'WCurrency_.[WCur_ID],
            WCurrency_.[WCur_Short],
            WCurrency_.[WCur_Desc],
            CAST(BINARY_CHECKSUM(WCurrency_.[WCur_ID],WCurrency_.[WCur_Short],WCurrency_.[WCur_Desc]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = 
            ' FROM ( ' +
                N'SELECT TOP 100 PERCENT IS_ROWNUM_COL, [WCur_ID] from @IS_TEMP_T_GETLIST ' +
                'WHERE IS_ROWNUM_COL >= '+ convert(varchar, @l_start_gen_row_num) + 
                ') IS_ALIAS LEFT JOIN ' +
                @l_from_str + ' ';

        SET @l_query_where = 
            N'ON WCurrency_.[WCur_ID] = IS_ALIAS.[WCur_ID] ' 
