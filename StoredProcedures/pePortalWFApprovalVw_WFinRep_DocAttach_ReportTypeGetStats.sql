if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalVw_WFinRep_DocAttach_ReportTypeGetStats') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalVw_WFinRep_DocAttach_ReportTypeGetStats 
GO
...
-- sum, avg, max, etc
CREATE PROCEDURE pePortalWFApprovalVw_WFinRep_DocAttach_ReportTypeGetStats
    ...
    BEGIN
        ...
        SET @l_temp_col_type_query = N'SELECT @data_type_out = DATA_TYPE, @precision_out = NUMERIC_PRECISION, @scale_out = NUMERIC_SCALE FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME=''vw_WFinRep_DocAttach_ReportType'' and COLUMN_NAME = ' + '''' + @l_temp_col_name + ''''
        ...
        BEGIN
            ...
            SET @l_query = 
                ...
                ') ' + 'vw_WFinRep_DocAttach_ReportType_'
        ...
        BEGIN
            SET @l_query = 
                'SELECT count(*) from ' + '[dbo].[vw_WFinRep_DocAttach_ReportType] vw_WFinRep_DocAttach_ReportType_ ' +
                'WHERE 1=2;'
            EXECUTE (@l_query)
        END
        SET NOCOUNT OFF
