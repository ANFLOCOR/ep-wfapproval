if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalWCanvass_PO_Map1GetStats') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalWCanvass_PO_Map1GetStats 
GO
...
-- sum, avg, max, etc
CREATE PROCEDURE pePortalWFApprovalWCanvass_PO_Map1GetStats
    ...
    BEGIN
        ...
        SET @l_temp_col_type_query = N'SELECT @data_type_out = DATA_TYPE, @precision_out = NUMERIC_PRECISION, @scale_out = NUMERIC_SCALE FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME=''WCanvass_PO_Map'' and COLUMN_NAME = ' + '''' + @l_temp_col_name + ''''
        ...
        BEGIN
            SET @l_query = 
                'SELECT count(*) from ' + '[dbo].[WCanvass_PO_Map] WCanvass_PO_Map_ ' +
                'WHERE 1=2;'
            EXECUTE (@l_query)
        END
        SET NOCOUNT OFF
