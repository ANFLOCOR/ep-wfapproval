if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalWClassification1GetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalWClassification1GetList 
GO
...
-- Returns a query resultset from table [dbo].[WClassification]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalWClassification1GetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[WClassification] WClassification_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY WClassification_.[WClass_ID] asc '
        ...
        SET @l_query_select = 'WITH WClassification_ AS ( SELECT  '
        SET @l_query_rownum = 'ROW_NUMBER() OVER(' + @l_sort_str + ') AS IS_ROWNUM_COL,'
        SET @l_query_cols = 
            N'WClassification_.[WClass_ID],
            WClassification_.[WClass_Name],
            WClassification_.[WClass_Value],
            CAST(BINARY_CHECKSUM(WClassification_.[WClass_ID],WClassification_.[WClass_Name],WClassification_.[WClass_Value]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = ' FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM WClassification_ '
    ...
    BEGIN
        ...
        SET @l_query_cols = 
            ...
            CAST(BINARY_CHECKSUM(WClassification_.[WClass_ID],WClassification_.[WClass_Name],WClassification_.[WClass_Value]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = 
            ' FROM [dbo].[WClassification] WClassification_ ' + 
            'WHERE 1=2;'
