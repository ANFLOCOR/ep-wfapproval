if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalWASS_Email_Default1GetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalWASS_Email_Default1GetList 
GO
...
-- Returns a query resultset from table [dbo].[WASS_Email_Default]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalWASS_Email_Default1GetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[WASS_Email_Default] WASS_Email_Default_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY WASS_Email_Default_.[WED_ID] asc '
        ...
        SET @l_query_select = 'WITH WASS_Email_Default_ AS ( SELECT  '
        SET @l_query_rownum = 'ROW_NUMBER() OVER(' + @l_sort_str + ') AS IS_ROWNUM_COL,'
        SET @l_query_cols = 
            N'WASS_Email_Default_.[WED_ID],
            WASS_Email_Default_.[WED_Type],
            WASS_Email_Default_.[WED_Template],
            CAST(BINARY_CHECKSUM(WASS_Email_Default_.[WED_ID],WASS_Email_Default_.[WED_Type],WASS_Email_Default_.[WED_Template]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = ' FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM WASS_Email_Default_ '
    ...
    BEGIN
        ...
        SET @l_query_cols = 
            ...
            CAST(BINARY_CHECKSUM(WASS_Email_Default_.[WED_ID],WASS_Email_Default_.[WED_Type],WASS_Email_Default_.[WED_Template]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = 
            ' FROM [dbo].[WASS_Email_Default] WASS_Email_Default_ ' + 
            'WHERE 1=2;'
