if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalVw_Conso_CFCategoriesGetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalVw_Conso_CFCategoriesGetList 
GO
...
-- Returns a query resultset from table [dbo].[vw_Conso_CFCategories]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalVw_Conso_CFCategoriesGetList
...
BEGIN
    ...
    BEGIN
        ...
        SET @l_query_select = 'WITH vw_Conso_CFCategories_ AS ( SELECT  '
        SET @l_query_rownum = 'ROW_NUMBER() OVER(' + @l_sort_str + ') AS IS_ROWNUM_COL,'
        SET @l_query_cols = 
            N'vw_Conso_CFCategories_.[ACCATROW_ID],
            vw_Conso_CFCategories_.[ACCATNUM],
            vw_Conso_CFCategories_.[ACCATDESC],
            vw_Conso_CFCategories_.[ORDERNO],
            vw_Conso_CFCategories_.[LEVELNO],
            vw_Conso_CFCategories_.[LEVEL1],
            vw_Conso_CFCategories_.[LEVEL1_ORD],
            vw_Conso_CFCategories_.[LEVEL2],
            vw_Conso_CFCategories_.[LEVEL2_ORD],
            vw_Conso_CFCategories_.[INDENT],
            vw_Conso_CFCategories_.[IsPrimary]'

        SET @l_query_from = 'FROM ' + @l_from_str + ' ' + @l_join_str + + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM vw_Conso_CFCategories_ '
    ...
    BEGIN
        ...
        SET @l_query_cols = 
            ...
            vw_Conso_CFCategories_.[IsPrimary]'
        SET @l_query_from = 
            ' FROM [dbo].[vw_Conso_CFCategories] vw_Conso_CFCategories_ ' + 
            'WHERE 1=2;'
