if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalReportGetStats') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalReportGetStats 
GO
...
-- sum, avg, max, etc
CREATE PROCEDURE pePortalWFApprovalReportGetStats
    ...
    BEGIN
        ...
        SET @l_temp_col_type_query = N'SELECT @data_type_out = DATA_TYPE, @precision_out = NUMERIC_PRECISION, @scale_out = NUMERIC_SCALE FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME=''Report'' and COLUMN_NAME = ' + '''' + @l_temp_col_name + ''''
        ...
        BEGIN
            ...
            ELSE
                SET @l_sort_str = N'ORDER BY Report_.[ReportID] asc '
        ...
        BEGIN
            SET @l_query = 
                'SELECT count(*) from ' + '[dbo].[Report] Report_ ' +
                'WHERE 1=2;'
            EXECUTE (@l_query)
        END
        SET NOCOUNT OFF
