if exists (select * from sysobjects where id = object_id(N'pePortalWFApprovalSysSetupUserApplication2GetList') and sysstat & 0xf = 4) drop procedure pePortalWFApprovalSysSetupUserApplication2GetList 
GO
...
-- Returns a query resultset from table [dbo].[SysSetupUserApplication]
...
--    1) count is 0 ;
--    2) empty resultset.
CREATE PROCEDURE pePortalWFApprovalSysSetupUserApplication2GetList
...
BEGIN
    ...
    SET @l_from_str = '[dbo].[SysSetupUserApplication] SysSetupUserApplication_'
    ...
    BEGIN
        ...
        ELSE
            SET @l_sort_str = N'ORDER BY SysSetupUserApplication_.[SSUA_RowID] asc '
        ...
        SET @l_query_select = 'WITH SysSetupUserApplication_ AS ( SELECT  '
        SET @l_query_rownum = 'ROW_NUMBER() OVER(' + @l_sort_str + ') AS IS_ROWNUM_COL,'
        SET @l_query_cols = 
            N'SysSetupUserApplication_.[SSUA_SSU_UserName],
            SysSetupUserApplication_.[SSUA_App_ID],
            SysSetupUserApplication_.[SSUA_DateCreated],
            SysSetupUserApplication_.[SSUA_RowID],
            CAST(BINARY_CHECKSUM(SysSetupUserApplication_.[SSUA_SSU_UserName],SysSetupUserApplication_.[SSUA_App_ID],SysSetupUserApplication_.[SSUA_DateCreated],SysSetupUserApplication_.[SSUA_RowID]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = ' FROM ' + @l_from_str + ' ' + @l_join_str + ' ' + @l_where_str + ') '
        SET @l_query_select2 = 'SELECT * FROM SysSetupUserApplication_ '
    ...
    BEGIN
        ...
        SET @l_query_cols = 
            ...
            CAST(BINARY_CHECKSUM(SysSetupUserApplication_.[SSUA_SSU_UserName],SysSetupUserApplication_.[SSUA_App_ID],SysSetupUserApplication_.[SSUA_DateCreated],SysSetupUserApplication_.[SSUA_RowID]) AS nvarchar(max)) AS IS_CHECKSUM_COLUMN_12345'
        SET @l_query_from = 
            ' FROM [dbo].[SysSetupUserApplication] SysSetupUserApplication_ ' + 
            'WHERE 1=2;'
